...
from models.models.land import LogisticsMeta, RoadQuality, FuelType, CargoAirportTier, RailNetworkType, FleetMaintenanceImpact, FuelTripEstimate, AirFreightConnectivity, RailFreightIntegration
...
_BASELINE_ANNUAL_MAINT_PER_TRUCK_EGP = 85000
_REFERENCE_FLEET_SIZE = 50
_DIESEL_PRICE_PER_LITER_EGP = 15.5
...
_CARGO_AIRPORT_REGISTRY: Dict[str, Dict] = {'Cairo International Airport': {'tier': CargoAirportTier.TIER_1_MAJOR, 'daily_capacity_tons': 1200.0, 'perishable_export': True}, 'Borg El Arab Airport': {'tier': CargoAirportTier.TIER_2_REGIONAL, 'daily_capacity_tons': 150.0, 'perishable_export': True}, 'Sphinx International Airport': {'tier': CargoAirportTier.TIER_2_REGIONAL, 'daily_capacity_tons': 80.0, 'perishable_export': True}, 'Aswan International Airport': {'tier': CargoAirportTier.TIER_2_REGIONAL, 'daily_capacity_tons': 40.0, 'perishable_export': False}}
_RAIL_COST_SAVINGS: Dict[RailNetworkType, float] = {RailNetworkType.HIGH_SPEED_ELECTRIC: 40.0, RailNetworkType.CONVENTIONAL_FREIGHT: 25.0, RailNetworkType.NONE: 0.0}
...
class LogisticsService:
    ...
    def analyze(self, land: Dict) -> LogisticsMeta:
        ...
        return LogisticsMeta(nearest_highway_km=raw.get('nearest_highway_km'), road_quality=road_quality, nearest_cargo_airport_km=raw.get('nearest_cargo_airport_km'), rail_freight_access=raw.get('rail_freight_access', False), rail_station_name=raw.get('rail_station_name', ''), estimated_fuel_cost_per_trip_egp=raw.get('estimated_fuel_cost_per_trip_egp'), nearest_port_name=raw.get('nearest_port_name', ''), container_handling_nearby=raw.get('container_handling_nearby', False), cold_chain_available=raw.get('cold_chain_available', False), avg_truck_turnaround_hours=raw.get('avg_truck_turnaround_hours'), accessibility_score=accessibility, logistics_verdict=verdict, fleet_maintenance=fleet_maintenance, fuel_trip=fuel_trip, air_freight=air_freight, rail_freight=rail_freight)
    ...
    def rank_for_logistics(self, lands: List[Dict], top_k: int=5) -> List[Dict]:
        ...
        for land in lands:
            ...
            results.append({'land_id': land['Land_ID'], 'governorate': land['Governorate'], 'region': land['Region_City'], 'usage': land['Allowed_Usage'], 'accessibility_score': analysis.accessibility_score, 'verdict': analysis.logistics_verdict, 'highlights': highlights, 'logistics': analysis})
    ...
    def generate_logistics_report(self, land: Dict) -> str:
        ...
        if analysis.fuel_trip:
            ...
            if ft.solar_savings_pct > 0:
                lines.append(f'  Solar-Assisted Savings:   {ft.solar_savings_pct:.1f}% (Hybrid fleet reduces fuel cost to {ft.cost_per_trip_egp * (1 - ft.solar_savings_pct / 100):,.0f} EGP/trip)')
        ...
        if analysis.air_freight:
            ...
            lines.append(f"  Perishable Export:      {('Yes' if af.perishable_export_suitable else 'No')}")
        else:
        ...
        if analysis.rail_freight:
            ...
            if rf.estimated_tonnage_cost_saving_pct > 0:
                lines.append(f'  Tonnage Cost Saving:     {rf.estimated_tonnage_cost_saving_pct:.0f}% vs truck-only')
            lines.append(f"  Heavy-Tonnage Viable:    {('Yes' if rf.heavy_tonnage_viable else 'No')} (threshold: {_HEAVY_TONNAGE_THRESHOLD_TONS_PER_DAY:.0f} tons/day)")
        else:
            lines.append(f"  Rail Freight Access:     {('Yes' if analysis.rail_freight_access else 'No')}")
        lines.extend(['', 'WAREHOUSING SUPPORT:'])
        lines.append(f"  Container Handling Nearby:  {('Yes' if analysis.container_handling_nearby else 'No')}")
        lines.append(f"  Cold Chain Available:       {('Yes' if analysis.cold_chain_available else 'No')}")
        ...
        if usage in ('Logistics', 'Industrial'):
            if analysis.accessibility_score < 30:
                lines.extend(['', 'LOGISTICS SUITABILITY WARNING:', f'  This land scores {analysis.accessibility_score:.0f}/100 on logistics accessibility. For a logistics or warehousing project, this is below the recommended threshold of 40. High transportation costs and limited multimodal access may significantly impact operating margins.'])
            elif analysis.accessibility_score >= 70:
                lines.extend(['', 'LOGISTICS SUITABILITY: STRONG', f'  Excellent multimodal access (score {analysis.accessibility_score:.0f}/100). This location is well-positioned for distribution centers, container storage, and cross-docking operations.'])
        return '\n'.join(lines)
    ...
    def generate_logistics_feasibility_matrix(self, land: Dict) -> str:
        ...
        region = f"{land['Governorate']} - {land['Region_City']}"
        usage = land.get('Allowed_Usage', '')
        lines = [f'LOGISTICS FEASIBILITY MATRIX -- {land_id} ({region})', f'Usage: {usage} | Accessibility Score: {analysis.accessibility_score:.0f}/100', '=' * 65, '', '  DIMENSION              | METRIC                          | VALUE', '  ' + '-' * 63]
        if analysis.fleet_maintenance:
            fm = analysis.fleet_maintenance
            lines.append(f'  Fleet Maintenance      | Road Quality Index              | {fm.road_quality_index.value}')
            lines.append(f'  Fleet Maintenance      | Maintenance Overhead            | +{fm.maintenance_overhead_pct:.0f}%')
            if fm.estimated_annual_maintenance_egp is not None:
                lines.append(f'  Fleet Maintenance      | Annual Cost ({_REFERENCE_FLEET_SIZE} trucks)         | {fm.estimated_annual_maintenance_egp:>10,.0f} EGP')
        else:
            lines.append('  Fleet Maintenance      | Road Quality Index              | N/A')
        if analysis.fuel_trip:
            ft = analysis.fuel_trip
            lines.append(f'  Fuel Consumption       | Fuel Type                       | {ft.fuel_type.value}')
            lines.append(f'  Fuel Consumption       | Distance to Port               | {ft.distance_km:.0f} km')
            lines.append(f'  Fuel Consumption       | Consumption (one-way)           | {ft.consumption_liters:.1f} L')
            lines.append(f'  Fuel Consumption       | Cost per Round-Trip             | {ft.cost_per_trip_egp:,.0f} EGP')
            lines.append(f'  Fuel Consumption       | Cost per Ton-km                 | {ft.cost_per_ton_km_egp:.2f} EGP')
            lines.append(f'  Fuel Consumption       | Solar Savings Potential         | {ft.solar_savings_pct:.1f}%')
            lines.append(f'  Fuel Consumption       | Annual Fuel Cost                | {ft.annual_fuel_cost_egp:,.0f} EGP')
        else:
            fuel = analysis.estimated_fuel_cost_per_trip_egp
            if fuel is not None:
                lines.append(f'  Fuel Consumption       | Cost per Port Trip             | {fuel:,.0f} EGP')
        if analysis.air_freight:
            af = analysis.air_freight
            lines.append(f"  Air Freight            | Nearest Cargo Airport           | {af.nearest_cargo_airport or 'N/A'}")
            lines.append(f'  Air Freight            | Airport Tier                    | {af.airport_tier.value}')
            lines.append(f'  Air Freight            | Trucking Transit Time           | {af.trucking_transit_hours:.1f} hours')
            cap = f'{af.daily_cargo_capacity_tons:,.0f} tons' if af.daily_cargo_capacity_tons else 'N/A'
            lines.append(f'  Air Freight            | Daily Cargo Capacity            | {cap}')
            lines.append(f"  Air Freight            | Perishable Export               | {('Yes' if af.perishable_export_suitable else 'No')}")
        else:
            lines.append('  Air Freight            | Nearest Cargo Airport           | N/A')
        if analysis.rail_freight:
            rf = analysis.rail_freight
            lines.append(f"  Rail Freight           | Rail Access                     | {('Yes' if rf.rail_access else 'No')}")
            lines.append(f'  Rail Freight           | Network Type                    | {rf.network_type.value}')
            if rf.station_name:
                lines.append(f'  Rail Freight           | Station / Distance              | {rf.station_name} ({rf.station_distance_km:.0f} km)' if rf.station_distance_km else f'  Rail Freight           | Station                         | {rf.station_name}')
            lines.append(f'  Rail Freight           | Tonnage Cost Saving             | {rf.estimated_tonnage_cost_saving_pct:.0f}%')
            lines.append(f"  Rail Freight           | Heavy-Tonnage Viable            | {('Yes' if rf.heavy_tonnage_viable else 'No')}")
        else:
            lines.append(f"  Rail Freight           | Rail Access                     | {('Yes' if analysis.rail_freight_access else 'No')}")
        lines.append('  ' + '-' * 63)
        lines.append(f"  Warehousing            | Container Handling              | {('Yes' if analysis.container_handling_nearby else 'No')}")
        lines.append(f"  Warehousing            | Cold Chain Available             | {('Yes' if analysis.cold_chain_available else 'No')}")
        if analysis.avg_truck_turnaround_hours is not None:
            lines.append(f'  Warehousing            | Truck Turnaround                | {analysis.avg_truck_turnaround_hours:.1f} hours')
    ...
    def format_logistics_for_llm(self, land: Dict) -> str:
        ...
        if analysis.accessibility_score == 0.0 and (not analysis.rail_freight_access):
            return ''
        parts = [f'  Logistics Score: {analysis.accessibility_score:.0f}/100', f"  Highway: {analysis.nearest_highway_km:.1f}km | Road: {analysis.road_quality.value} | Rail: {('Yes' if analysis.rail_freight_access else 'No')}"]
        if analysis.fleet_maintenance and analysis.fleet_maintenance.maintenance_overhead_pct > 0:
            parts.append(f'  Fleet Maint Overhead: +{analysis.fleet_maintenance.maintenance_overhead_pct:.0f}%')
        if analysis.fuel_trip and analysis.fuel_trip.annual_fuel_cost_egp > 0:
            parts.append(f'  Annual Fuel: {analysis.fuel_trip.annual_fuel_cost_egp:,.0f} EGP | Per Ton-km: {analysis.fuel_trip.cost_per_ton_km_egp:.2f} EGP')
        elif analysis.estimated_fuel_cost_per_trip_egp:
            parts.append(f"  Port Trip Fuel: {analysis.estimated_fuel_cost_per_trip_egp:,.0f} EGP | Turnaround: {analysis.avg_truck_turnaround_hours or 'N/A'}h")
        if analysis.air_freight and analysis.air_freight.nearest_cargo_airport:
            af = analysis.air_freight
            parts.append(f'  Air Cargo: {af.nearest_cargo_airport} ({af.airport_tier.value}, {af.trucking_transit_hours:.1f}h transit)')
        if analysis.rail_freight and analysis.rail_freight.rail_access:
            rf = analysis.rail_freight
            parts.append(f'  Rail: {rf.network_type.value} at {rf.station_name} (-{rf.estimated_tonnage_cost_saving_pct:.0f}% vs truck)')
    ...
    @staticmethod
    def _compute_fleet_maintenance(road_quality: RoadQuality, land: Dict) -> FleetMaintenanceImpact:
        """
        ...
        annual_cost = round(_BASELINE_ANNUAL_MAINT_PER_TRUCK_EGP * _REFERENCE_FLEET_SIZE * multiplier)
        return FleetMaintenanceImpact(road_quality_index=road_quality, maintenance_overhead_pct=overhead_pct, estimated_annual_maintenance_egp=annual_cost, wear_factors=wear_factors)

    @staticmethod
    def _compute_fuel_consumption(raw: Dict, road_quality: RoadQuality, land: Dict) -> FuelTripEstimate:
        """
        ...
        degradation = 1.0 + (1.0 - road_factor) * 0.3
        ...
        return FuelTripEstimate(fuel_type=fuel_type, distance_km=port_km, consumption_liters=round(one_way_liters, 1), cost_per_trip_egp=round(cost_per_trip), cost_per_ton_km_egp=round(cost_per_ton_km, 2), solar_savings_pct=solar_savings, annual_fuel_cost_egp=round(annual_fuel_cost))
    ...
    def _compute_air_freight_connectivity(raw: Dict) -> AirFreightConnectivity:
        ...
        return AirFreightConnectivity(nearest_cargo_airport=airport_name, airport_tier=airport_tier, distance_km=distance_km or 0.0, trucking_transit_hours=round(transit_hours, 1), daily_cargo_capacity_tons=daily_capacity, perishable_export_suitable=bool(perishable))
    ...
    def _compute_rail_freight(raw: Dict) -> RailFreightIntegration:
        ...
        if not heavy_tonnage and rail_access:
            if network_type == RailNetworkType.HIGH_SPEED_ELECTRIC:
                ...
                heavy_tonnage = True
        return RailFreightIntegration(rail_access=rail_access, network_type=network_type, station_name=station_name, station_distance_km=station_distance, estimated_tonnage_cost_saving_pct=cost_saving, heavy_tonnage_viable=heavy_tonnage)
    ...
    def _generate_verdict(accessibility: float, raw: Dict, land: Dict) -> str:
        ...
        usage = land.get('Allowed_Usage', '')
        if accessibility >= 75:
            verdict = 'Prime logistics hub location with excellent multimodal connectivity. Ideal for distribution centers and bonded warehousing.'
        elif accessibility >= 55:
            verdict = 'Strong logistics accessibility with good highway and port connections. Suitable for most warehousing and freight operations.'
        elif accessibility >= 35:
            verdict = 'Moderate logistics access. Functional for regional distribution but may incur higher per-unit transportation costs.'
        elif accessibility >= 20:
            verdict = 'Limited logistics infrastructure. Road or rail upgrades may be needed for commercial-scale freight operations.'
        else:
            verdict = 'Poor logistics accessibility. Significant investment in transportation links required before viable for logistics use.'
