...
class RecommendationEngine:
    ...
    def generate_recommendations(
        ...
        for land, pred in zip(lands, predictions):
            ...
            # ── Infrastructure completeness (0-20 points) ──
            ...
            # ── Density & Clustering Factor (0-15 points) ──
            density_analysis = self.density_svc.analyze(land)
            density_score = 0.0

            # Reward low-saturation areas (greenfield advantage)
            ...
            # Penalize if land usage matches a saturated category
            ...
            # Reward gap opportunities
            if density_analysis.market_gap_analysis:
                aligned_gaps = [
                    g for g in density_analysis.market_gap_analysis
                    if g.get("current_usage_viable") and "WARNING" not in str(g.get("current_usage_viable", ""))
                ]
                if aligned_gaps:
            ...
            # ── Logistics & Warehousing Factor (0-15 points, for Logistics/Industrial) ──
            ...
            if usage in ("Logistics", "Industrial"):
                ...
                # Air freight Tier-1 advantage
                if (logistics_analysis.air_freight
                        and logistics_analysis.air_freight.airport_tier.value == "Tier-1 Major"
                        and logistics_analysis.air_freight.trucking_transit_hours <= 1.5):
                    log_score = min(log_score + 1, 15)
            ...
            scored.append({
                ...
                "usage": land["Allowed_Usage"],
                "current_price_sqm": land["Price_Per_Sqm_EGP"],
                ...
                "density_score": density_analysis.overall_density_score,
                "density_verdict": density_analysis.clustering_verdict,
                "logistics_score": logistics_analysis.accessibility_score,
                "logistics_verdict": logistics_analysis.logistics_verdict,
            })
    ...
    def format_recommendation_text(self, rec: Dict) -> str:
        ...
        lines = [
            f"{tag} {rec['land_id']} — {rec['governorate']}: {rec['region']}",
            f"  Usage: {rec['usage']} | Current: {rec['current_price_sqm']:,.0f} EGP/m\u00b2",
            f"  6-Month Forecast: +{rec['predicted_change_pct']:.1f}% (confidence: {rec['confidence']:.0f}%)",
            f"  Urgency: {rec['urgency']} (Score: {rec['action_score']}/100)",
        ]
