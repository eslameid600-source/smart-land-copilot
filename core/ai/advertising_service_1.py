...
class AdvertisingService:
    ...
    def create_campaign(
        ...
        seller_id: str,
        campaign_type: str,
        target_channels: Optional[List[AdChannel]] = None,
        target_audience: str = "",
        total_budget_egp: float = 0.0,
        delegated_to_broker_id: Optional[str] = None,
    ) -> AdvertisingCampaign:
        """
        ...
        campaign = AdvertisingCampaign(
            ...
            delegated_to_broker_id=delegated_to_broker_id,
            status=CampaignStatus.DRAFT,
        )

        self._campaigns[campaign_id] = campaign
        logger.info(
            f"Campaign {campaign_id} created: {campaign_type} for land {land_id}"
        )
        return campaign
    ...
    def get_campaign_stats(self) -> Dict:
        ...
        return {
            ...
            "total_budget_egp": sum(c.total_budget_egp for c in campaigns),
            "total_spent_egp": sum(c.spent_egp for c in campaigns),
    ...
    # ────────────────────────────────────────────────────────────
    # Option A: AI Copilot Ad Generation
    # ────────────────────────────────────────────────────────────

    def generate_ai_copilot_ads(
        self,
        campaign_id: str,
        land_metadata: Dict,
        ...
        for channel in campaign.target_channels:
            if channel == AdChannel.SEO_META:
                continue
            social_copy[channel.value] = self._generate_channel_copy(
        ...
        seo_meta = self._generate_seo_meta(
            land_id=land_id,
            governorate=governorate,
            region=region,
            area=area,
    ...
    def _generate_channel_copy(
        ...
        if channel == AdChannel.FACEBOOK.value:
            return (
                f"Investment Opportunity: {area:,} sqm ({area_feddan} feddan) {usage} Land — {location_str}\n\n"
                f"Price: {price_sqm:,} EGP/sqm | Total: {total_price:,.0f} EGP\n\n"
                f"Infrastructure: {utilities}\n"
                f"Highway Access: {highways}\n\n"
                f"{notes[:200] if notes else 'Prime location with strong growth potential in the Egyptian real estate market.'}\n\n"
                f"Contact us for due diligence reports and site visits. "
                f"Verified title deeds. Shahr Eqary registered.\n\n"
                f"#EgyptRealEstate #{usage}Land #{governorate.replace(' ', '')}Investment "
                f"#LandForSale #SmartLandCopilot"
            )

        elif channel == AdChannel.LINKEDIN.value:
            return (
                f"Strategic Land Acquisition Opportunity — {location_str}\n\n"
                f"We are pleased to present a {area:,} sqm ({area_feddan} feddan) "
                f"{usage}-zoned parcel positioned for {usage.lower()} development.\n\n"
                f"Key Metrics:\n"
                f"  - Price per sqm: {price_sqm:,} EGP\n"
                f"  - Total asking price: {total_price:,.0f} EGP\n"
                f"  - Zoning: {usage}\n"
                f"  - Utilities: {utilities}\n"
                f"  - Highway access: {highways}\n\n"
                f"{notes[:250] if notes else 'This parcel offers compelling ROI potential backed by Egyptian government development incentives and growing infrastructure.'}\n\n"
                f"Ref: {land_id} | Due diligence available upon request.\n\n"
                f"#PropTech #EgyptInvestment #{usage} #LandAcquisition #RealEgypt"
            )

        elif channel == AdChannel.X_TWITTER.value:
            return (
                f"{area:,} sqm {usage} land in {region}, {governorate} — "
                f"{price_sqm:,} EGP/sqm ({total_price:,.0f} EGP total). "
                f"Utilities: {utilities}. {highways}. "
                f"Verified title. DM for details. "
                f"#EgyptRealEstate #Land"
            )

        elif channel == AdChannel.INSTAGRAM.value:
            return (
                f"Premium {usage} Land — {location_str}\n\n"
                f"{area:,} sqm | {price_sqm:,} EGP/sqm\n"
                f"{total_price:,.0f} EGP total\n\n"
                f"Infrastructure: {utilities}\n"
                f"Access: {highways}\n\n"
                f"DM for investment brief & site visit scheduling.\n\n"
                f"#EgyptRealEstate #LandInvestment #{usage} #{governorate.replace(' ', '')}Land"
            )

        elif channel == AdChannel.GOOGLE_SEARCH.value:
            return (
                f"Buy {usage} Land in {region}, {governorate} — "
                f"{area:,} sqm at {price_sqm:,} EGP/sqm. "
                f"Total: {total_price:,.0f} EGP. "
                f"{utilities}. {highways}. Verified title deeds. "
                f"View details and schedule a site visit today."
            )
    ...
    def _generate_seo_meta(
        ...
        location_str = f"{region}, {governorate}"
        title = (
            f"{usage} Land for Sale in {location_str} — "
            f"{area:,} sqm at {price_sqm:,} EGP/sqm | Smart Land Copilot"
        )
        description = (
            f"Explore this {area:,} sqm {usage.lower()}-zoned land in {location_str}, Egypt. "
            f"Priced at {price_sqm:,} EGP per square meter with {utilities} infrastructure. "
            f"Access via {highways}. Verified title deeds. "
            f"View investment analysis, feasibility reports, and schedule site visits."
        )
        keywords = (
            f"{usage.lower()} land for sale, {governorate.lower()} real estate, "
            f"{region.lower()} land investment, Egypt {usage.lower()} property, "
            f"{price_sqm:,} EGP per sqm, {area:,} sqm land, "
            f"Egyptian real estate market, Smart Land Copilot"
        )
        og_title = f"{usage} Land — {location_str} | {area:,} sqm"
        og_description = (
            f"{area:,} sqm {usage} parcel in {location_str} at {price_sqm:,} EGP/sqm. "
            f"Full infrastructure: {utilities}. Invest in Egypt's growing real estate market."
        )
    ...
    # ────────────────────────────────────────────────────────────
    # Option B: Platform-Managed Campaigns (Simulated)
    # ────────────────────────────────────────────────────────────

    def deploy_paid_campaign(
        self,
        campaign_id: str,
        impressions: int = 0,
        clicks: int = 0,
...
def _seed_sample_campaigns(svc: AdvertisingService) -> None:
    ...
    for land in lands[:3]:
        camp = svc.create_campaign(
            ...
            seller_id="USR-00000002",
            campaign_type="ai_copilot",
            target_audience="Investors and developers looking for premium Egyptian land",
        )
        svc.generate_ai_copilot_ads(camp.campaign_id, land)
        svc.activate_campaign(camp.campaign_id)

        svc.deploy_paid_campaign(
            camp.campaign_id,
            impressions=4500 + hash(land["Land_ID"]) % 5000,
            clicks=120 + hash(land["Land_ID"]) % 200,
            leads=8 + hash(land["Land_ID"]) % 15,
            spent=3500.0 + hash(land["Land_ID"]) % 5000,
        )