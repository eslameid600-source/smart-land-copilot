...
from models.auction import (AuctionRecord, AuctionStatus, Bid, BidStatus,
                            LandLead, LeadStatus, ListingSource,
...
class LandSourcingService:
    ...
    def get_lead_stats(self) -> Dict:
        ...
        return {
            ...
            "submitted": sum(1 for lead in leads if lead.status == LeadStatus.SUBMITTED),
            "documents_uploaded": sum(1 for lead in leads if lead.status == LeadStatus.DOCUMENTS_UPLOADED),
            "notary_verified": sum(1 for lead in leads if lead.status == LeadStatus.NOTARY_VERIFIED),
            "rejected": sum(1 for lead in leads if lead.status == LeadStatus.REJECTED),
            "scout_fee_eligible": sum(1 for lead in leads if lead.scout_fee_eligible),
        }
