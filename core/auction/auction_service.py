...
from models.models.auction import AuctionRecord, AuctionStatus, Bid, BidStatus, TransactionFeeBreakdown, LandLead, LeadStatus, ListingSource
...
class AuctionEngine:
    ...
    def create_auction(self, land_id: str, governorate: str, region_city: str, total_area_sqm: int, allowed_usage: str, base_price_egp: float, auction_end_date: Optional[str]=None, auction_start_date: Optional[str]=None, minimum_increment_pct: float=2.0, reserve_price_egp: Optional[float]=None, listing_source: ListingSource=ListingSource.OWNER_DIRECT, scout_id: Optional[str]=None, scout_name: Optional[str]=None) -> AuctionRecord:
        """
        ...
        record = AuctionRecord(auction_id=auction_id, land_id=land_id, governorate=governorate, region_city=region_city, total_area_sqm=total_area_sqm, allowed_usage=allowed_usage, base_price_egp=base_price_egp, base_price_per_sqm_egp=base_per_sqm, auction_start_date=auction_start_date or datetime.now().isoformat(), auction_end_date=auction_end_date, minimum_increment_pct=minimum_increment_pct, reserve_price_egp=reserve_price_egp, listing_source=listing_source, scout_id=scout_id, scout_name=scout_name, status=AuctionStatus.PENDING)
    ...
    def place_bid(self, auction_id: str, bidder_id: str, bidder_name: str, bid_amount_egp: float, is_auto_bid: bool=False) -> Tuple[Optional[Bid], str]:
        """
        ...
        if bid_amount_egp < minimum:
            return (None, f'Bid must be at least {minimum:,.0f} EGP (current highest: {auction.current_highest_bid_egp:,.0f} EGP, min increment: {auction.minimum_increment_pct}%)')
        ...
        bid = Bid(bid_id=bid_id, auction_id=auction_id, bidder_id=bidder_id, bidder_name=bidder_name, bid_amount_egp=bid_amount_egp, bid_per_sqm_egp=bid_per_sqm, bid_timestamp=datetime.now().isoformat(), status=BidStatus.PENDING, is_auto_bid=is_auto_bid)
        ...
        logger.info(f'Bid {bid_id}: {bidder_name} -> {bid_amount_egp:,.0f} EGP on {auction_id} ({bid_per_sqm:,.2f} EGP/sqm)')
        return (bid, '')
    ...
    def _finalize_auction(self, auction: AuctionRecord) -> None:
        ...
        if auction.winning_bid:
            ...
            if auction.reserve_price_egp and auction.current_highest_bid_egp < auction.reserve_price_egp:
                logger.info(f'Auction {auction.auction_id} ended below reserve ({auction.current_highest_bid_egp:,.0f} < {auction.reserve_price_egp:,.0f})')
            else:
                logger.info(f'Auction {auction.auction_id} won by {auction.winning_bid.bidder_name} at {auction.current_highest_bid_egp:,.0f} EGP')
    ...
    def list_auctions(self, status: Optional[AuctionStatus]=None, governorate: Optional[str]=None, usage: Optional[str]=None) -> List[AuctionRecord]:
...
class CommissionCalculator:
    ...
    @classmethod
    def compute_breakdown(cls, total_value_egp: float, land_id: str='', auction_id: Optional[str]=None, is_auction: bool=False, scout_name: str='', scout_eligible: bool=False, custom_platform_pct: Optional[float]=None, custom_scout_pct: Optional[float]=None) -> TransactionFeeBreakdown:
        """
        ...
        """
        breakdown = TransactionFeeBreakdown(land_id=land_id, auction_id=auction_id, total_transaction_value_egp=total_value_egp, transaction_type='Auction' if is_auction else 'Direct Sale', platform_commission_pct=custom_platform_pct or cls.DEFAULT_PLATFORM_PCT, scout_fee_pct=custom_scout_pct or cls.DEFAULT_SCOUT_PCT, scout_name=scout_name, scout_eligible=scout_eligible, real_estate_disposal_tax_pct=cls.DISPOSAL_TAX_PCT, registration_notary_fee_pct=cls.REGISTRATION_NOTARY_PCT, stamp_duty_pct=cls.STAMP_DUTY_PCT)
        breakdown.compute()
        return breakdown
    ...
    def compute_for_auction(cls, auction: AuctionRecord) -> TransactionFeeBreakdown:
        ...
        return cls.compute_breakdown(total_value_egp=sale_value, land_id=auction.land_id, auction_id=auction.auction_id, is_auction=True, scout_name=auction.scout_name or '', scout_eligible=auction.listing_source == ListingSource.SCOUT_SOURCED)

    @classmethod
    def compute_for_direct_sale(cls, land: Dict, scout_name: str='', scout_eligible: bool=False) -> TransactionFeeBreakdown:
        """Convenience method: compute breakdown from a raw land dict."""
        sale_value = land['Total_Area_Sqm'] * land['Price_Per_Sqm_EGP']
        return cls.compute_breakdown(total_value_egp=sale_value, land_id=land['Land_ID'], is_auction=False, scout_name=scout_name, scout_eligible=scout_eligible)
    ...
    def format_breakdown_table(cls, breakdown: TransactionFeeBreakdown) -> List[Dict]:
        ...
        """
        rows = [{'Party / Item': 'SELLER — Gross Receipt', 'Rate': '100.00%', 'Amount (EGP)': f'{breakdown.seller_gross_receipt_egp:,.2f}', 'Notes': 'Full transaction value'}, {'Party / Item': '  (-) Platform Commission', 'Rate': f'-{breakdown.platform_commission_pct:.1f}%', 'Amount (EGP)': f'-{breakdown.platform_commission_egp:,.2f}', 'Notes': 'Marketplace transaction fee'}]
        if breakdown.scout_eligible and breakdown.scout_fee_egp > 0:
            rows.append({'Party / Item': f'  (-) Scout Fee ({breakdown.scout_name})', 'Rate': f'-{breakdown.scout_fee_pct:.1f}%', 'Amount (EGP)': f'-{breakdown.scout_fee_egp:,.2f}', 'Notes': 'Sourcing agent discovery fee'})
        rows.extend([{'Party / Item': '  (-) Real Estate Disposal Tax', 'Rate': f'-{breakdown.real_estate_disposal_tax_pct:.1f}%', 'Amount (EGP)': f'-{breakdown.real_estate_disposal_tax_egp:,.2f}', 'Notes': 'ضريبة التصرفات العقارية'}, {'Party / Item': '  (-) Registration / Notary Fees', 'Rate': f'-{breakdown.registration_notary_fee_pct:.1f}%', 'Amount (EGP)': f'-{breakdown.registration_notary_fee_egp:,.2f}', 'Notes': 'الشهر العقاري + الموثق العمومي'}, {'Party / Item': '  (-) Stamp Duty', 'Rate': f'-{breakdown.stamp_duty_pct:.1f}%', 'Amount (EGP)': f'-{breakdown.stamp_duty_egp:,.2f}', 'Notes': 'واجبة الختم'}, {'Party / Item': 'SELLER — Net Proceeds', 'Rate': f'{breakdown.seller_effective_pct:.2f}%', 'Amount (EGP)': f'{breakdown.seller_net_proceeds_egp:,.2f}', 'Notes': 'Effective seller take-home'}, {'Party / Item': '---', 'Rate': '---', 'Amount (EGP)': '---', 'Notes': '---'}, {'Party / Item': 'BUYER — Total Cost', 'Rate': '', 'Amount (EGP)': f'{breakdown.buyer_total_cost_egp:,.2f}', 'Notes': 'Transaction value + all government duties'}, {'Party / Item': '  (+) Transaction Value', 'Rate': '', 'Amount (EGP)': f'{breakdown.total_transaction_value_egp:,.2f}', 'Notes': ''}, {'Party / Item': '  (+) Total Government Duties', 'Rate': f'{breakdown.real_estate_disposal_tax_pct + breakdown.registration_notary_fee_pct + breakdown.stamp_duty_pct:.1f}%', 'Amount (EGP)': f'+{breakdown.total_government_duties_egp:,.2f}', 'Notes': 'Disposal Tax + Registration + Stamp Duty'}])
        return rows
...
class LandSourcingService:
    ...
    def submit_lead(self, scout_id: str, scout_name: str, governorate: str, region_city: str, estimated_area_sqm: Optional[int], estimated_price_per_sqm_egp: Optional[float], soil_type: str='', allowed_usage: str='', nearest_highways: str='', utilities_availability: str='', description: str='') -> LandLead:
        """Submit a new land lead in Scout Mode."""
        lead_id = f'LEAD-{uuid.uuid4().hex[:8].upper()}'
        lead = LandLead(lead_id=lead_id, scout_id=scout_id, scout_name=scout_name, governorate=governorate, region_city=region_city, estimated_area_sqm=estimated_area_sqm, estimated_price_per_sqm_egp=estimated_price_per_sqm_egp, soil_type=soil_type, allowed_usage=allowed_usage, nearest_highways=nearest_highways, utilities_availability=utilities_availability, description=description, status=LeadStatus.SUBMITTED, scout_fee_eligible=False)
        self._leads[lead_id] = lead
        logger.info(f'Land lead {lead_id} submitted by scout {scout_name}')
        return lead

    def upload_legal_documents(self, lead_id: str, land_id: Optional[str]=None) -> Optional[LandLead]:
        """
    ...
    def verify_with_notary(self, lead_id: str, notary_reference_number: str='') -> Optional[LandLead]:
        """
        ...
        logger.info(f'Lead {lead_id} verified by Notary (ref: {notary_reference_number}), scout fee eligible for {lead.scout_name}')
        return lead
    ...
    def list_leads(self, scout_id: Optional[str]=None, status: Optional[LeadStatus]=None) -> List[LandLead]:
...
def _initialize_sample_auctions(engine: AuctionEngine) -> None:
    ...
    for land in lands:
        if land['Investment_Status'] == 'Public Auction':
            ...
            auction = engine.create_auction(land_id=land['Land_ID'], governorate=land['Governorate'], region_city=land['Region_City'], total_area_sqm=land['Total_Area_Sqm'], allowed_usage=land['Allowed_Usage'], base_price_egp=starting_total, auction_end_date=land.get('Auction_Date'), minimum_increment_pct=2.0, reserve_price_egp=round(starting_total * 1.15, 2))
...
def _initialize_sample_leads(service: LandSourcingService) -> None:
    ...
    service.submit_lead(scout_id='SCOUT-005', scout_name='Ahmed Mansour', governorate='Suez', region_city='Ain Sokhna Industrial Zone', estimated_area_sqm=75000, estimated_price_per_sqm_egp=2200, soil_type='Sandy limestone', allowed_usage='Industrial', nearest_highways='Cairo-Sokhna Road, Galala Mountain Road', utilities_availability='Water, Electricity', description='Industrial zone expansion parcel near Sokhna port. Owner is a retiring factory owner looking to liquidate. Strong container logistics potential.')
    service.upload_legal_documents('LEAD-00000001', land_id=None)
    service.submit_lead(scout_id='SCOUT-007', scout_name='Fatma Hassan', governorate='Ismailia', region_city='Ismailia City West', estimated_area_sqm=40000, estimated_price_per_sqm_egp=4500, soil_type='Sandy clay', allowed_usage='Logistics', nearest_highways='Ismailia-Cairo Agricultural Road', utilities_availability='Water, Electricity, Gas', description='Prime logistics site adjacent to the Suez Canal corridor. Suitable for bonded warehouse development. Existing road access is excellent.')