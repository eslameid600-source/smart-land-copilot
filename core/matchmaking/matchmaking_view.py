...
from ui.components import render_compatibility_bar, render_auction_card, render_match_card, compat_color
...
def render_matchmaking_view() -> None:
    ...
    if results:
        ...
        st.markdown(f"""<div style="background:linear-gradient(135deg,#0d3b0d,#1a5c1a);border:2px solid {compat_color(compat)};border-radius:12px;padding:16px;margin-bottom:12px;text-align:center;"><div style="font-size:12px;color:#aaa;">TOP MATCH</div><div style="font-size:24px;font-weight:bold;color:{compat_color(compat)};">{top.get('Land_ID', '')} — {compat}%</div><div style="font-size:13px;color:#bbb;">{top.get('Governorate', '')} — {top.get('Region_City', '')}</div></div>""", unsafe_allow_html=True)
    ...
    with col_btn:
        analyze_btn = st.button('AI Analysis', type='primary', use_container_width=True, disabled=state.matchmaking.llm_analysis_done)
    with col_status:
...
def _run_matchmaking(criteria: InvestorCriteria, state) -> None:
    ...
    state.matchmaking.set_results(report_data=display_results, criteria_summary=report.criteria_summary, context_text=matchmaking_svc.format_for_llm(report))

def _run_llm_analysis(state) -> None:
    """Send matchmaking results to GLM for AI analysis."""
    glm_service = get_glm_service()
    state.chat.add_user_message(f'[Matchmaking Analysis] Criteria: {state.matchmaking.criteria_summary}')
    with st.chat_message('assistant'):
        response_stream = st.write_stream(glm_service.stream_matchmaking(criteria_summary=state.matchmaking.criteria_summary, context_text=state.matchmaking.context_text))
