...
class MatchmakingService:
    ...
    def match(
        ...
        if results:
            top = results[0]
            top_rec = (
                f"{top.land_id} ({top.compatibility_percent}%) — "
                f"{top.recommendation.value}"
            )

        report = MatchmakingReport(
            criteria_summary=criteria.to_display_string(),
            results=results,
            total_lands_analyzed=len(lands),
            top_recommendation=top_rec,
        )

        logger.info(
            "Matchmaking complete: %d lands analyzed, top=%s (%.1f%%)",
            len(lands),
            results[0].land_id if results else "N/A",
            results[0].compatibility_percent if results else 0,
        )
        return report
    ...
    def format_for_llm(self, report: MatchmakingReport) -> str:
        ...
        lines = [
            "RANKED LAND RESULTS (Matchmaking Compatibility)",
            "=" * 60,
        ]
        ...
        for result in report.results:
            ...
            lines.append(
                ...
                f"  Location       : {r.get('Governorate', 'N/A')} - {r.get('Region_City', 'N/A')}\n"
                f"  Area           : {r.get('Total_Area_Sqm', 'N/A'):,} sqm\n"
                f"  Price/sqm      : {r.get('Price_Per_Sqm_EGP', 'N/A'):,} EGP\n"
                f"  Usage          : {r.get('Allowed_Usage', 'N/A')}\n"
                f"  Risk Level     : {result.investment_risk.value}\n"
                f"  Recommendation : {result.recommendation.value}\n"
                f"  Strengths      : {'; '.join(result.strengths) or 'None identified'}\n"
                f"  Weaknesses     : {'; '.join(result.weaknesses) or 'None identified'}\n"
            )

            if result.is_auction:
                lines.append(
                    f"  *** AUCTION *** Date: {r.get('Auction_Date', 'N/A')}, "
                    f"Starting: {r.get('Starting_Price_Per_Sqm_EGP', 'N/A'):,} EGP/m2\n"
                )
    ...
    def _score_land(
        self, land: LandRecord, criteria: InvestorCriteria
    ) -> MatchResult:
    ...
    @staticmethod
    def _score_usage(
        land: LandRecord, criteria: InvestorCriteria, max_score: float
    ) -> tuple[float, ScoreBreakdown]:
        """Score usage type match."""
        if not criteria.target_usage:
            return max_score, ScoreBreakdown(
                category="Usage Match", max_score=max_score,
                actual_score=max_score, passed=True,
                detail="No preference specified",
            )
        if land.allowed_usage == criteria.target_usage:
            return max_score, ScoreBreakdown(
                category="Usage Match", max_score=max_score,
                actual_score=max_score, passed=True,
                detail=f"Exact match: {land.allowed_usage}",
            )
        return 0.0, ScoreBreakdown(
            category="Usage Match", max_score=max_score,
            actual_score=0.0, passed=False,
            detail=f"Want {criteria.target_usage}, got {land.allowed_usage}",
        )
    ...
    def _score_area(
        ...
        if not criteria.min_area_sqm:
            return max_score, ScoreBreakdown(
                category="Area Match", max_score=max_score,
                actual_score=max_score, passed=True,
        ...
        if land.total_area_sqm >= criteria.min_area_sqm:
            return max_score, ScoreBreakdown(
                category="Area Match", max_score=max_score,
                actual_score=max_score, passed=True,
                detail=f"{land.total_area_sqm:,} >= {criteria.min_area_sqm:,} sqm",
            )
        ratio = land.total_area_sqm / criteria.min_area_sqm
        partial = ratio * max_score
        return partial, ScoreBreakdown(
            category="Area Match", max_score=max_score,
            actual_score=round(partial, 1), passed=False,
            detail=f"{land.total_area_sqm:,} < {criteria.min_area_sqm:,} sqm ({ratio:.0%} of required)",
        )
    ...
    def _score_price(
        ...
        if not criteria.max_price_per_sqm:
            return max_score, ScoreBreakdown(
                category="Price Match", max_score=max_score,
                actual_score=max_score, passed=True,
                detail="No budget specified",
            )
        if land.price_per_sqm_egp <= criteria.max_price_per_sqm:
            return max_score, ScoreBreakdown(
                category="Price Match", max_score=max_score,
                actual_score=max_score, passed=True,
                detail=f"{land.price_per_sqm_egp:,} <= {criteria.max_price_per_sqm:,} EGP/sqm",
            )
        ...
        return partial, ScoreBreakdown(
            category="Price Match", max_score=max_score,
            actual_score=round(partial, 1), passed=False,
            detail=f"{land.price_per_sqm_egp:,} > {criteria.max_price_per_sqm:,} EGP/sqm ({ratio:.0%} of budget)",
        )
    ...
    def _score_utilities(
        ...
        if not criteria.required_utilities:
            return max_score, ScoreBreakdown(
                category="Utilities Match", max_score=max_score,
                actual_score=max_score, passed=True,
                detail="No utility preference",
            )
    ...
    def _score_auction(
        ...
        if land.is_auction:
            detail = (
                f"Auction on {land.auction_date} "
                f"(starting {land.starting_price_per_sqm_egp:,} EGP/m2)"
            )
            return max_score, ScoreBreakdown(
                category="Auction Opportunity", max_score=max_score,
                actual_score=max_score, passed=True, detail=detail,
            )
        return max_score * 0.5, ScoreBreakdown(
            category="Auction Opportunity", max_score=max_score,
            actual_score=round(max_score * 0.5, 1), passed=False,
            detail="Direct sale (no auction)",
        )

    # ----------------------------------------------------------
    # Risk & Recommendation Classification
    # ----------------------------------------------------------

    @staticmethod
    def _classify_risk(
        land: LandRecord, compatibility: float, weaknesses: List[str]
    ) -> RiskLevel:
    ...
    def _classify_recommendation(
        compatibility: float, risk: RiskLevel, weaknesses: List[str]
    ) -> RecommendationType:
