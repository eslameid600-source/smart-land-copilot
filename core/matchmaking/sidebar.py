...
from models.models.land import ALL_UTILITIES
...
def _render_dashboard_metrics(analytics) -> None:
    ...
    render_metric_card('Total Area', f'{stats.total_area_sqm:,} sqm', '#2ecc71')
    render_metric_card('Avg Price/sqm', f'{stats.avg_price_per_sqm:,.0f} EGP', '#e74c3c')
...
def _render_map_filter() -> None:
    ...
    new_filter = st.radio('Show on map:', options=categories, index=categories.index(current) if current in categories else 0, label_visibility='collapsed', horizontal=False)
...
def _render_matchmaking_form(state) -> None:
    ...
    with st.form('matchmaking_form', clear_on_submit=False):
        target_usage = st.selectbox('Target Usage Type', options=['(Any)'] + ['Industrial', 'Agricultural', 'Logistics', 'Residential'], format_func=lambda x: 'Any Type' if x == '(Any)' else x)
        col_area, col_price = st.columns(2)
        with col_area:
            min_area = st.number_input('Min Area (sqm)', min_value=0, max_value=10000000, value=0, step=10000)
        with col_price:
            max_price = st.number_input('Max Price (EGP/sqm)', min_value=0, max_value=100000, value=0, step=100)
        required_utils = st.multiselect('Required Utilities', options=ALL_UTILITIES, default=[])
        submitted = st.form_submit_button('Match My Criteria', type='primary', use_container_width=True)
        if submitted:
            criteria = InvestorCriteria(target_usage=target_usage if target_usage != '(Any)' else None, min_area_sqm=min_area if min_area > 0 else None, max_price_per_sqm=max_price if max_price > 0 else None, required_utilities=required_utils)
