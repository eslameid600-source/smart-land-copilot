...
from models.matchmaking import MatchResult, RiskLevel, RecommendationType, ScoreBreakdown
...
def config():
    return AppConfig(
        mock_mode=True,
        matchmaking_weights=MatchmakingWeights(),
    )
...
def auction_land_dict(sample_land_dict):
    ...
    d["Starting_Price_Per_Sqm_EGP"] = 4_500
    return d
...
class TestInvestorCriteria:
    ...
    def test_validation_passes(self):
        criteria = InvestorCriteria(min_area_sqm=100_000, max_price_per_sqm=5000)
    ...
    def test_to_dict_roundtrip(self):
        criteria = InvestorCriteria(
            target_usage="Logistics",
            min_area_sqm=200_000,
            max_price_per_sqm=3000,
            required_utilities=["Water"],
        )
        d = criteria.to_dict()
        restored = InvestorCriteria.from_dict(d)
        assert restored.target_usage == "Logistics"
        assert restored.min_area_sqm == 200_000
...
class TestMatchmakingScoring:
    ...
    def test_perfect_match_high_score(self, sample_land_dict, auction_land_dict):
        ...
        criteria = InvestorCriteria(
            ...
            required_utilities=["Water", "Electricity", "Gas", "Fiber-Optic"],
        )
    ...
    def test_wrong_usage_lower_score(self, sample_land_dict, auction_land_dict):
        svc = self._make_service([sample_land_dict, auction_land_dict])
        criteria = InvestorCriteria(
            target_usage="Agricultural",
            min_area_sqm=100_000,
            max_price_per_sqm=10_000,
        )
        report = svc.match(criteria)
        for result in report.results:
            assert result.compatibility_percent < 80  # Usage mismatch

    def test_area_too_small_partial_credit(self, sample_land_dict, auction_land_dict):
        svc = self._make_service([sample_land_dict, auction_land_dict])
        criteria = InvestorCriteria(min_area_sqm=300_000)
        report = svc.match(criteria)
        for result in report.results:
            area_bd = next(
                (sb for sb in result.score_breakdown if sb.category == "Area Match"), None
            )
            if area_bd:
    ...
    def test_price_over_budget_partial_credit(self, sample_land_dict, auction_land_dict):
        ...
        for result in report.results:
            price_bd = next(
                (sb for sb in result.score_breakdown if sb.category == "Price Match"), None
            )
            if price_bd:
    ...
    def test_missing_utility_penalty(self, sample_land_dict, auction_land_dict):
        ...
        for result in report.results:
            util_bd = next(
                (sb for sb in result.score_breakdown if sb.category == "Utilities Match"), None
            )
            if util_bd:
    ...
    def test_auction_bonus(self, sample_land_dict, auction_land_dict):
        ...
        report = svc.match(criteria)
        auction_result = next(
            (r for r in report.results if r.land_id == "TEST-AUC"), None
        )
        direct_result = next(
            (r for r in report.results if r.land_id == "TEST-01"), None
        )
        assert auction_result is not None
        assert direct_result is not None
        auction_bd = next(
            (sb for sb in auction_result.score_breakdown if sb.category == "Auction Opportunity"), None
        )
        direct_bd = next(
            (sb for sb in direct_result.score_breakdown if sb.category == "Auction Opportunity"), None
        )
    ...
    def test_no_criteria_full_credit(self, sample_land_dict, auction_land_dict):
        ...
        report = svc.match(criteria)
        for result in report.results:
            assert result.compatibility_percent >= 90  # Only auction bonus differs
    ...
    def test_strengths_and_weaknesses_populated(self, sample_land_dict, auction_land_dict):
        ...
        criteria = InvestorCriteria(
            target_usage="Agricultural",
            min_area_sqm=300_000,
            required_utilities=["Sewage"],
        )
        report = svc.match(criteria)
        for result in report.results:
            # Should have at least some strengths or weaknesses
