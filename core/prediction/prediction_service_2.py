...
class PredictionService:
    ...
    # ── Governorate price multipliers (based on demand/supply) ──
    GOV_MULTIPLIERS: Dict[str, float] = {
        "Cairo": 1.35,
        "Alexandria": 1.15,
        "Sharqia": 1.10,
        "Suez": 1.20,
    ...
    # ── Usage type growth multipliers ──
    USAGE_GROWTH: Dict[str, float] = {
        "Residential": 0.12,    # 12% annual growth
        "Industrial": 0.08,
        "Logistics": 0.14,      # Driven by e-commerce
        "Agricultural": 0.05,
    }

    # ── Infrastructure premium ──
    INFRA_PREMIUMS: Dict[str, float] = {
        "Fiber_Optic": 0.05,
        "Gas_Pipeline": 0.03,
        "Sewage_Connection": 0.02,
        "Railway_Access": 0.03,
    }
    ...
    def predict(
        ...
        # Weighted combination: historical (35%), usage type (20%),
        # governorate demand (15%), trend (10%), inflation (5%),
        # infrastructure (5%), density clustering (10%)
        annual_growth = (
            hist_growth_1y * 0.35
            + usage_growth * 0.20
            + (gov_mult - 1.0) * 0.15
        ...
        if change_pct > 15 and confidence > 75:
            rec = (
                f"STRONG BUY SIGNAL — Expected {change_pct:.1f}% appreciation "
                f"over {horizon_months} months with {confidence:.0f}% confidence. "
                f"Consider acting now before further price increases."
            )
        elif change_pct > 8:
            rec = (
                f"FAVORABLE — Projected {change_pct:.1f}% growth. "
                f"Good entry point for {horizon_months}-month horizon."
            )
        elif change_pct > 0:
            rec = (
                f"NEUTRAL — Modest {change_pct:.1f}% growth expected. "
                f"Evaluate against alternative investment opportunities."
            )
        else:
            rec = (
                f"CAUTION — Projected decline of {abs(change_pct):.1f}%. "
                f"Monitor market conditions before committing."
            )
        ...
        return PricePrediction(
            ...
            predicted_price_per_sqm=round(projected_price, 2),
        ...
        for pred in predictions:
            ...
            if land:
                heatmap.append({
                    "lat": land["Latitude"],
                    "lon": land["Longitude"],
                    "current_price": pred.current_price_per_sqm,
                    "predicted_price": pred.predicted_price_per_sqm,
                    "change_pct": pred.predicted_change_pct,
                    "intensity": max(0.1, pred.predicted_change_pct / 20.0),
                    "land_id": pred.land_id,
                    "governorate": pred.governorate,
                    "region": pred.region_city,
                })
        return heatmap
    ...
    def _compute_density_adjustment(land: Dict) -> float:
        ...
        # Sigmoid-like scaling: more services → more appreciation support
        # Capped at ~40 total to avoid extreme values
        normalized = min(total / 40.0, 1.0)

        # Map to growth adjustment: -0.03 (empty) to +0.07 (dense cluster)
        adjustment = -0.03 + normalized * 0.10
