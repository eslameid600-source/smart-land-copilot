...
from models.models.models.land import LandSummary
...
class AnalyticsService:
    ...
    def get_summary_dict(self) -> Dict[str, Any]:
        ...
        return {'total_lands': s.total_lands, 'total_area_sqm': s.total_area_sqm, 'avg_price_per_sqm': s.avg_price_per_sqm, 'auction_lands': s.auction_lands, 'direct_sale_lands': s.direct_sale_lands, 'usage_breakdown': s.usage_breakdown, 'governorate_breakdown': s.governorate_breakdown}
    ...
    def get_auction_summary(self) -> List[Dict[str, Any]]:
        ...
        return [{'Land_ID': land.land_id, 'Governorate': land.governorate, 'Region_City': land.region_city, 'Allowed_Usage': land.allowed_usage, 'Total_Area_Sqm': land.total_area_sqm, 'Price_Per_Sqm_EGP': land.price_per_sqm_egp, 'Auction_Date': land.auction_date, 'Starting_Price_Per_Sqm_EGP': land.starting_price_per_sqm_egp} for land in auction_lands]
    ...
    def get_area_range(self, usage: Optional[str]=None) -> Dict[str, int]:
        ...
        return {'min': min(areas), 'max': max(areas), 'avg': round(sum(areas) / len(areas))}
