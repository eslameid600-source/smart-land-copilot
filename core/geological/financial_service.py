...
from models.models.models.financial import FinancialAnalysis, TaxBreakdown, CashFlowEntry
...
class FinancialService:
    ...
    SURVEY_FEE = 15000.0
    REVENUE_RATES: Dict[str, float] = {'Residential': 2500.0, 'Industrial': 800.0, 'Logistics': 600.0, 'Agricultural': 150.0}
    OPEX_RATIOS: Dict[str, float] = {'Residential': 0.25, 'Industrial': 0.35, 'Logistics': 0.3, 'Agricultural': 0.45}
    DISCOUNT_RATE = 0.15

    @classmethod
    def compute_full_analysis(cls, land: Dict, investment_horizon: int=5, discount_rate: Optional[float]=None) -> FinancialAnalysis:
        """
        ...
        infra_prep = area * dev_cost_sqm * 0.15
        taxes = TaxBreakdown(registration_fee_pct=cls.LAND_REGISTRATION_FEE_PCT, registration_fee_egp=round(registration_fee, 2), stamp_duty_pct=cls.STAMP_DUTY_PCT, stamp_duty_egp=round(stamp_duty, 2), notary_fees_egp=cls.NOTARY_FEE, legal_review_egp=cls.LEGAL_REVIEW_FEE, survey_fees_egp=cls.SURVEY_FEE, infrastructure_prep_egp=round(infra_prep, 2))
        taxes.total_acquisition_cost_egp = round(registration_fee + stamp_duty + cls.NOTARY_FEE + cls.LEGAL_REVIEW_FEE + cls.SURVEY_FEE, 2)
        ...
        opex_ratio = cls.OPEX_RATIOS.get(usage, 0.3)
        ...
        cash_flows.append(CashFlowEntry(year=0, revenue=0, operating_cost=0, net_cash_flow=-total_investment, cumulative_cash_flow=round(cumulative, 2), discount_factor=1.0, discounted_cash_flow=round(-total_investment, 2)))
        ...
        for year in range(1, investment_horizon + 1):
            ...
            if cumulative >= 0 and payback_year == investment_horizon:
                ...
                if prev_cum < 0 and year_net > 0:
                    ...
                    payback_year = year
            cash_flows.append(CashFlowEntry(year=year, revenue=round(year_revenue, 2), operating_cost=round(year_opex, 2), net_cash_flow=round(year_net, 2), cumulative_cash_flow=round(cumulative, 2), discount_factor=round(discount_factor, 4), discounted_cash_flow=round(year_dcf, 2)))
        ...
        return FinancialAnalysis(land_id=land['Land_ID'], land_price_egp=land_price, total_area_sqm=area, price_per_sqm_egp=price_sqm, taxes=taxes, development_cost_per_sqm=dev_cost_sqm, total_development_cost_egp=round(total_dev, 2), total_investment_egp=round(total_investment, 2), annual_revenue_egp=round(annual_revenue, 2), annual_operating_cost_egp=round(annual_opex, 2), annual_net_income_egp=round(annual_net, 2), roi_pct=round(roi, 2), irr_pct=round(irr, 2), payback_years=round(payback_year, 1), npv_egp=round(npv, 2), profit_margin_pct=round(profit_margin, 2), cash_flows=cash_flows, recommendation=rec, risk_flags=risk_flags)

    @staticmethod
    def _revenue_growth_rate(usage: str, year: int) -> float:
        """Annual revenue growth assumptions by usage type."""
        base_rates = {'Residential': 0.08, 'Industrial': 0.06, 'Logistics': 0.1, 'Agricultural': 0.04}
    ...
    def _compute_irr(cash_flows: List[CashFlowEntry], max_iter: int=1000) -> float:
        ...
        def npv_at_rate(r: float) -> float:
            return sum((cf.discounted_cash_flow * (1 / (1 + r) ** cf.year) if cf.year > 0 else cf.net_cash_flow for cf in cash_flows))
        rate = 0.1
        for _ in range(max_iter):
            npv_val = npv_at_rate(rate)
            if abs(npv_val) < 0.01:
                return rate * 100
            dnpv = sum((-cf.year * cf.net_cash_flow / (1 + rate) ** (cf.year + 1) if cf.year > 0 else cf.net_cash_flow for cf in cash_flows))
            if abs(dnpv) < 1e-10:
                break
            rate -= npv_val / dnpv
            rate = max(-0.99, min(rate, 5.0))
