...
class TestRAGService:
    ...
    def test_proactive_match(self):
        ...
        results = proactive_match(target_usage='Industrial', min_area=100000, max_price_per_sqm=5000, required_utilities=['Gas'])
        assert len(results) > 0
...
class TestFinancialService:
    ...
    def test_compute_full_analysis(self):
        ...
        assert len(analysis.cash_flows) == 6
...
class TestETLService:
    ...
    def test_ingest_json(self):
        ...
        etl = ETLService()
        json_data = '[{"Land_ID":"TEST-01","Governorate":"Test","Region_City":"TestCity","Latitude":30.0,"Longitude":31.0,"Total_Area_Sqm":10000,"Price_Per_Sqm_EGP":1000,"Allowed_Usage":"Industrial"}]'
    ...
    def test_validation_missing_field(self):
        from services.etl_service import ETLService
        etl = ETLService()
        json_data = '[{"Land_ID":"TEST-02","Governorate":"Test"}]'
        result = etl.ingest_json(json_data)
    ...
    def test_validation_bad_coords(self):
        from services.etl_service import ETLService
        etl = ETLService()
        json_data = '[{"Land_ID":"TEST-03","Governorate":"Test","Region_City":"Test","Latitude":50.0,"Longitude":31.0,"Total_Area_Sqm":1000,"Price_Per_Sqm_EGP":100,"Allowed_Usage":"Industrial"}]'
        result = etl.ingest_json(json_data)
        assert len(result['errors']) > 0
...
class TestPydanticModels:
    ...
    def test_financial_analysis_model(self):
        from models.models.financial import FinancialAnalysis, CashFlowEntry
    ...
    def test_price_prediction_model(self):
        ...
        pred = PricePrediction(land_id='TEST', governorate='Cairo', region_city='Test', current_price_per_sqm=1000, predicted_price_per_sqm=1150, predicted_change_pct=15.0, confidence_pct=80.0)
...
class TestDensityService:
    ...
    def test_analyze_new_cairo(self):
        ...
        from data.land_database import get_land_by_id
        from models.models.land import SaturationLevel
        land = get_land_by_id('EG-CAI-01')
    ...
    def test_gap_analysis_saturated_retail(self):
        ...
        from services.density_service import DensityService
        from models.models.land import SaturationLevel
        land = {'Land_ID': 'TEST-DENSITY', 'Allowed_Usage': 'Industrial', 'Service_Density_Clusters': [{'radius_km': 5, 'retail': 15, 'civic': 2, 'industrial': 1}]}
        result = DensityService().analyze(land)
    ...
    def test_analyze_no_density_data(self):
        ...
        from services.density_service import DensityService
        from models.models.land import SaturationLevel
        land = {'Land_ID': 'TEST-EMPTY', 'Allowed_Usage': 'Residential'}
        result = DensityService().analyze(land)
    ...
    def test_profile_totals(self):
        ...
        for land_id, analysis in results.items():
            for profile in analysis.profiles:
                expected_total = profile.retail_entertainment + profile.civic_infrastructure + profile.industrial_infrastructure
                assert profile.total == expected_total, f'{land_id} @ {profile.radius_km}km: total={profile.total} != {expected_total}'
...
class TestLogisticsService:
    ...
    def test_analyze_no_logistics_data(self):
        ...
        from services.logistics_service import LogisticsService
        from models.models.land import RoadQuality
    ...
    def test_fleet_maintenance_poor_roads(self):
        ...
        assert any(('severe' in f.lower() or 'extreme' in f.lower() or 'pothole' in f.lower() for f in result.fleet_maintenance.wear_factors))
    ...
    def test_fleet_maintenance_cost_scales_with_fleet(self):
        ...
        assert toshka.fleet_maintenance.estimated_annual_maintenance_egp > sokhna.fleet_maintenance.estimated_annual_maintenance_egp

    def test_fuel_consumption_diesel(self):
        """Standard land without solar grid should use Diesel fuel type."""
        from services.logistics_service import LogisticsService
        from data.land_database import get_land_by_id
        from models.models.land import FuelType
        land = get_land_by_id('EG-SHQ-01')
        result = LogisticsService().analyze(land)
    ...
    def test_fuel_no_port_distance(self):
        ...
        from services.logistics_service import LogisticsService
        from models.models.land import FuelType
        land = {'Land_ID': 'TEST-NO-PORT', 'Allowed_Usage': 'Logistics', 'logistics_meta': {'road_quality': 'Excellent'}}
        result = LogisticsService().analyze(land)
        assert result.fuel_trip is not None
    ...
    def test_air_freight_sokhna(self):
        ...
        from services.logistics_service import LogisticsService
        from data.land_database import get_land_by_id
        from models.models.land import CargoAirportTier
        land = get_land_by_id('EG-SUE-01')
        result = LogisticsService().analyze(land)
    ...
    def test_rail_freight_high_speed_electric(self):
        ...
        from services.logistics_service import LogisticsService
        from data.land_database import get_land_by_id
        from models.models.land import RailNetworkType
        land = get_land_by_id('EG-SUE-01')
        result = LogisticsService().analyze(land)
    ...
    def test_rail_cost_saving_high_speed_vs_conventional(self):
        ...
        assert sokhna.rail_freight.estimated_tonnage_cost_saving_pct > sharqia.rail_freight.estimated_tonnage_cost_saving_pct
    ...
    def test_recommendation_includes_rail_signal(self):
        ...
        has_rail_signal = any(('rail' in reason.lower() and 'tonnage' in reason.lower() for reason in sokhna_recs[0]['reasons']))
    ...
    def test_recommendation_includes_maintenance_signal(self):
        ...
        sokhna_recs = [r for r in recs if r['land_id'] == 'EG-SUE-01']
        assert len(sokhna_recs) > 0
        has_maint_signal = any(('maintenance' in reason.lower() for reason in sokhna_recs[0]['reasons']))
    ...
    def test_auction_engine_create_and_bid(self):
        ...
        auction = engine.create_auction(land_id='TEST-001', governorate='Cairo', region_city='Test City', total_area_sqm=10000, allowed_usage='Industrial', base_price_egp=5000000)
        assert auction.auction_id == 'AUC-TEST-001'
        assert auction.status.value == 'Live'
        assert auction.base_price_egp == 5000000
        bid, error = engine.place_bid(auction_id=auction.auction_id, bidder_id='B-001', bidder_name='Test Bidder', bid_amount_egp=5200000)
        assert error == ''
        assert bid.bid_amount_egp == 5200000
        assert bid.status.value == 'Winning'
        assert auction.current_highest_bid_egp == 5200000
        assert auction.bid_count == 1
        assert auction.registered_bidders_count == 1
        bid2, error2 = engine.place_bid(auction_id=auction.auction_id, bidder_id='B-002', bidder_name='Second Bidder', bid_amount_egp=5400000)
        assert error2 == ''
    ...
    def test_auction_minimum_bid_enforced(self):
        ...
        from services.auction_service import AuctionEngine
        engine = AuctionEngine()
        auction = engine.create_auction(land_id='TEST-002', governorate='Alex', region_city='Borg', total_area_sqm=5000, allowed_usage='Logistics', base_price_egp=1000000, minimum_increment_pct=5.0)
        _, error = engine.place_bid(auction_id=auction.auction_id, bidder_id='B-003', bidder_name='Low Bidder', bid_amount_egp=1000000)
    ...
    def test_commission_calculator_direct_sale(self):
        ...
        bd = CommissionCalculator.compute_breakdown(total_value_egp=10000000, land_id='TEST-DIR', is_auction=False, scout_eligible=False)
        assert bd.total_transaction_value_egp == 10000000
        assert bd.platform_commission_egp == 250000
        assert bd.scout_fee_egp == 0
        assert bd.real_estate_disposal_tax_egp == 250000
        assert bd.registration_notary_fee_egp == 300000
        assert bd.stamp_duty_egp == 50000
        assert bd.total_government_duties_egp == 600000
        assert bd.seller_net_proceeds_egp == 10000000 - 250000 - 600000
        assert bd.buyer_total_cost_egp == 10000000 + 600000

    def test_commission_calculator_with_scout(self):
        """Test fee breakdown with scout sourcing fee."""
        from services.auction_service import CommissionCalculator
        bd = CommissionCalculator.compute_breakdown(total_value_egp=10000000, land_id='TEST-SCOUT', is_auction=True, scout_name='Ahmed Mansour', scout_eligible=True)
        assert bd.scout_fee_egp == 150000
        assert bd.scout_name == 'Ahmed Mansour'
        total_deductions = 250000 + 150000 + 600000
        assert bd.seller_net_proceeds_egp == 10000000 - total_deductions

    def test_commission_calculator_format_table(self):
        """Test table formatting returns correct structure."""
        from services.auction_service import CommissionCalculator
        bd = CommissionCalculator.compute_breakdown(total_value_egp=5000000)
    ...
    def test_land_sourcing_workflow(self):
        ...
        from models.models.auction import LeadStatus
        svc = LandSourcingService()
        lead = svc.submit_lead(scout_id='S-001', scout_name='Test Scout', governorate='Suez', region_city='Sokhna', estimated_area_sqm=50000, estimated_price_per_sqm_egp=3000, description='Test lead for logistics land')
