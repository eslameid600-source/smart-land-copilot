...
class LandRepository:
    ...
    def search(
        ...
        logger.debug("search() returned %d results (filters: %s)", len(results), {
            "usage": usage, "governorate": governorate,
            "min_area": min_area, "max_price": max_price,
        })
        return results
    ...
    def get_summary(self) -> LandSummary:
        ...
        return LandSummary(
            total_lands=len(self._records),
            total_area_sqm=total_area,
            avg_price_per_sqm=round(avg_price, 2),
            auction_lands=auction_count,
            direct_sale_lands=len(self._records) - auction_count,
            usage_breakdown=usage_counts,
            governorate_breakdown=gov_counts,
        )
    ...
    def get_governorate_dataframe(self) -> pd.DataFrame:
        ...
        rows = [
            ...
            for gov, count in sorted(
                summary.governorate_breakdown.items(), key=lambda x: x[1], reverse=True
            )
        ]
