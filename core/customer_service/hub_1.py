...
class HubResponse:
    ...
    def __init__(
        ...
        chatbot_confidence: float = 0.0,
        chatbot_category: str = "",
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "chatbot_confidence": round(self.chatbot_confidence, 4),
    ...
    def __repr__(self) -> str:
        return (
            f"HubResponse(auto_resolved={self.auto_resolved}, "
            f"escalated={self.escalated}, ticket={self.ticket_created}, "
            f"confidence={self.chatbot_confidence:.2f})"
        )
...
class CustomerServiceHub:
    ...
    def __init__(
        self,
        # Zendesk config
        zendesk_subdomain: str = "smartland",
        zendesk_email: str = "",
        zendesk_api_token: str = "",
        zendesk_demo_mode: bool = True,
        # WhatsApp config
        whatsapp_provider: str = Provider.TWILIO,
        ...
        # Chatbot config
        chatbot_confidence_threshold: float = 0.45,
        chatbot_escalate_threshold: float = 0.20,
        chatbot_use_llm: bool = False,
        # Survey config
        auto_survey_after_ticket: bool = True,
    ):
        """
        ...
        self.zendesk = ZendeskClient(
            ...
            demo_mode=zendesk_demo_mode,
        )

        self.whatsapp = WhatsAppService(
            provider=whatsapp_provider,
            demo_mode=whatsapp_demo_mode,
            account_sid=whatsapp_account_sid,
            auth_token=whatsapp_auth_token,
            whatsapp_number=whatsapp_number,
            access_token=whatsapp_meta_token,
            phone_number_id=whatsapp_phone_number_id,
        )

        self.chatbot = RAGChatbot(
            confidence_threshold=chatbot_confidence_threshold,
            escalate_threshold=chatbot_escalate_threshold,
            use_llm=chatbot_use_llm,
        )
    ...
    def handle_message(
        ...
        logger.info(
            f"Handling message | channel={channel} | "
            f"query='{query[:60]}' | user={user_email or phone_number}"
        )

        # Step 1: Run through RAG chatbot
        user_context = {
            "user_id": user_id,
        ...
        if not chatbot_response.should_escalate and chatbot_response.confidence >= self.chatbot.confidence_threshold:
            # ── Case A: Auto-resolve (high confidence) ──
            ...
            return HubResponse(
                ...
                chatbot_confidence=chatbot_response.confidence,
                chatbot_category=chatbot_response.category,
                metadata={
                    "source_id": chatbot_response.source_id,
                    "action": "auto_resolved",
                },
            )

        # ── Case B or C: Need ticket (low/medium confidence) ──
        ticket = self._create_ticket_from_query(
            query=query,
            user_email=user_email,
            user_name=user_name,
        ...
        if ticket:
            ...
            # If escalation — notify via WhatsApp if phone available
            if chatbot_response.should_escalate and phone_number:
                self._send_escalation_whatsapp(
        ...
        return HubResponse(
            ...
            channel=channel,
            auto_resolved=False,
            escalated=chatbot_response.should_escalate,
            ...
            metadata={
                "source_id": chatbot_response.source_id,
                "action": "escalated" if chatbot_response.should_escalate else "partial_answer_with_ticket",
            },
        )
    ...
    def handle_whatsapp_message(
        ...
        if not conv:
            self.whatsapp.start_conversation(
                from_number, context={"profile_name": profile_name}
            )

        # Handle via main handler
        ...
        # If ticket created, send confirmation
        if response.ticket_created:
            self.whatsapp.send_template(
                to=from_number,
                template_key="ticket_created",
                params={"ticket_number": response.ticket_id or "N/A"},
            )

        # If escalated, send interactive options
        if response.escalated:
            self.whatsapp.send_interactive_buttons(
                to=from_number,
                body="تم تحويلك لوكيل الدعم. هل تريد إضافة أي معلومات إضافية؟",
                options=[
                    {"id": "add_info", "title": "إضافة معلومات"},
                    {"id": "urgent", "title": "طلب عاجل"},
    ...
    def handle_webhook(
        self, payload: Dict[str, Any], provider: str = "meta"
    ) -> List[HubResponse]:
        """
        ...
        for event in events:
            if event.event_type == "message" and event.message_body:
                response = self.handle_whatsapp_message(
                    from_number=event.phone_number,
    ...
    def survey_user(
        ...
        Returns:
            نتيجة التسجيل
        """
        result = self.survey.survey_user(
            transaction_id=transaction_id,
            rating=rating,
        ...
        logger.info(
            f"Survey recorded | txn={transaction_id} | "
            f"rating={rating} | success={result['success']}"
        )

        return result

    def survey_after_ticket_resolution(
        self,
        ticket_id: str,
        rating: int,
        user_email: str = "",
        ...
        return self.survey_user(
            transaction_id=f"TKT-{ticket_id}",
            rating=rating,
            survey_type=SurveyType.POST_TICKET,
            user_email=user_email,
            comment=comment,
    ...
    def resolve_ticket(
        ...
        if ticket_id.startswith("TK-"):
            # It's our external ID — find zendesk_id
        ...
        if self.auto_survey_after_ticket:
            # Create survey invitation (would be sent via WhatsApp/email)
            survey_result = self.survey.create_invitation(
                transaction_id=f"TKT-{ticket_id}",
                survey_type=SurveyType.POST_TICKET,
                channel="web",
            )
        ...
        return {
            ...
            "survey_invitation": survey_result,
        }
    ...
    def get_dashboard(self, days: int = 30) -> Dict[str, Any]:
        ...
        return {
            "hub": {
                "total_messages_handled": self._total_handled,
            ...
            "zendesk": zendesk_metrics,
            "whatsapp": whatsapp_metrics,
            "survey": survey_dashboard,
        }
    ...
    def _create_ticket_from_query(
        ...
        # Determine subject from query (first 100 chars)
        subject = query[:100] if len(query) <= 100 else query[:97] + "..."

        # Enrich description
        description_parts = [
            f"الاستفسار الأصلي: {query}",
            "",
            f"القناة: {channel}",
            f"البريد: {user_email or 'غير محدد'}",
        ]
        ...
        if chatbot_response.source_id:
            ...
            description_parts.append(
                f"نص FAQ: {chatbot_response.source_question}"
            )
        if transaction_id:
        ...
        ticket = ZendeskTicket(
            ...
            requester_email=user_email or "unknown@example.com",
            requester_name=user_name,
            priority=priority,
            tags=[
                "smart-land",
                f"channel-{channel}",
                "chatbot-escalated" if chatbot_response.should_escalate else "chatbot-partial",
            ],
        )
        ...
        try:
            ...
            logger.info(
                f"Ticket created | external_id={result.get('external_id')} | "
                f"zendesk_id={result.get('zendesk_id')} | "
                f"escalated={chatbot_response.should_escalate}"
            )
            return result
        except ZendeskAPIError as e:
    ...
    def _send_escalation_whatsapp(
        self, phone_number: str, ticket_id: str, user_name: str
    ) -> None:
        """إرسال إشعار تحويل عبر واتساب"""
        name = user_name or "العميل"
        message = (
            f"مرحباً {name}،\n\n"
            f"تم تحويل استفسارك إلى أحد وكلاء الدعم المتخصصين.\n"
            f"رقم التذكرة: {ticket_id}\n"
            f"سيتم التواصل معك في أقرب وقت ممكن.\n\n"
            f"شكراً لصبرك! 🙏"
        )
        try:
    ...
    def _find_zendesk_id_by_external(self, external_id: str) -> Optional[int]:
        ...
        # In demo mode, search local storage
        ...
        # In live mode, use Zendesk search API
        try:
    ...
    def run_demo(self) -> Dict[str, Any]:
        ...
        demo_queries = [
            ("كيف أبدأ استخدام المنصة؟", "web"),
            ("ما هي طرق الدفع المتاحة؟", "web"),
            ("كيف يتم تصنيف جودة الأراضي؟", "web"),
            ("مشكلة خطيرة في المزاد لا أستطيع تقديم عرض", "whatsapp"),
            ("أريد استرداد أموالي فوراً", "whatsapp"),
            ("سؤال عن شيء غير موجود في القاعدة", "web"),
        ]
        ...
        for query, channel in demo_queries:
            ...
            results.append({
                ...
                "confidence": response.chatbot_confidence,
            })
        ...
        return {
            ...
            "surveys_recorded": len(survey_results),
            "dashboard": dashboard,
        }