...
class CustomerServiceSystem:
    ...
    def __init__(self):
        ...
        self._chat_counts: Dict[str, int] = {}  # session_id -> chat count
    ...
    def create_ticket(
        ...
        # Track chat count for satisfaction survey trigger
    ...
    def get_dashboard_metrics(self) -> Dict:
        ...
        return {
            ...
            "open_tickets": len([t for t in all_t if t.status == TicketStatus.OPEN]),
            "escalated_tickets": len([t for t in all_t if t.status == TicketStatus.ESCALATED]),
            "resolved_tickets": len([t for t in all_t if t.status == TicketStatus.RESOLVED]),
            "avg_satisfaction": self.get_satisfaction_stats()["avg_score"],
            "auto_escalation_rate": (
                len([t for t in all_t if t.auto_escalated]) / max(len(all_t), 1) * 100
            ),
        }
    ...
    def _classify_category(query: str) -> str:
        ...
        query_lower = query.lower()
        categories = {
            "Financial": ["price", "cost", "tax", "roi", "irr", "budget", "payment", "fee"],
            "Legal": ["law", "legal", "permit", "license", "contract", "regulation", "nuca", "gafi"],
            "Technical": ["map", "data", "api", "error", "bug", "system", "loading"],
            "Market Analysis": ["trend", "predict", "forecast", "market", "appreciation"],
            "Land Info": ["soil", "geological", "infrastructure", "utility", "water", "electricity"],
            "General Inquiry": [],
        }
