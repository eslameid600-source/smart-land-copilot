...
# ────────────────────────────────────────────────────────────
# 0. BACKWARD-COMPATIBLE RE-EXPORTS from services/rag_service
# ────────────────────────────────────────────────────────────

from services.rag_service import (                     # noqa: F401
    extract_intent,
    search_lands,
    proactive_match as _legacy_proactive_match,
    filter_lands_by_usage,
    format_context_for_llm,
    _generate_fee_breakdown_inline,
    _format_auction_status_inline,
    USAGE_ALIASES,
    GOVERNORATE_ALIASES,
)

# Re-export legacy proactive_match under its original name
# for backward compatibility with existing imports
proactive_match = _legacy_proactive_match


# ────────────────────────────────────────────────────────────
# 1. QUALITY RATING WEIGHT MAP
# ────────────────────────────────────────────────────────────
...
# ────────────────────────────────────────────────────────────
# 2. ADVANCED COMPATIBILITY SCORING ENGINE
# ────────────────────────────────────────────────────────────
...
def compute_compatibility_score(
    ...
    if criteria.target_usage:
        ...
        scores["usage"] = 25.0  # No preference = full score
        reasons.append("No usage preference specified")
    total += scores["usage"]

    # ── 2. Area Adequacy (20 pts) ──
    ...
    return MatchResult(
        ...
        auction_date=land.get("Auction_Date"),
        starting_price_per_sqm=land.get("Starting_Price_Per_Sqm_EGP"),
        land_quality_rating=quality,
        price_affordability_ratio=scores.get("price_ratio", 1.0),
        area_adequacy_ratio=scores.get("area_ratio", 1.0),
        utility_coverage_pct=scores.get("util_pct", 100.0),
    )


# ────────────────────────────────────────────────────────────
# 3. INVESTOR SMART MATCH (Main Entry Point)
# ────────────────────────────────────────────────────────────

def investor_smart_match(
    criteria: InvestorCriteria,
    top_k: int = 10,
    min_score: float = 0.0,
) -> List[MatchResult]:
    ...
    for land in lands:
        # Optional: pre-filter by minimum quality rating
...
# ────────────────────────────────────────────────────────────
# 4. TRIPARTITE CLASSIFICATION: SELLERS
# ────────────────────────────────────────────────────────────

def classify_seller(
    owned_land_ids: List[str],
    lands_data: Optional[List[Dict]] = None,
) -> SellerProfile:
    """
    ...
    profile = SellerProfile(
        seller_id="",
        owned_land_ids=valid_ids,
        total_asset_count=len(valid_ids),
        total_listed_value_egp=total_value,
    )
    profile.classify()
    return profile


def classify_all_sellers(
    ...
    for account in user_accounts:
        ...
        results.append({
            ...
            "asset_count": profile.total_asset_count,
            "total_value_egp": profile.total_listed_value_egp,
            "volume_tier": profile.volume_tier.value,
            "value_tier": profile.value_tier.value,
            "combined_label": profile.combined_label,
        })
...
# ────────────────────────────────────────────────────────────
# 5. TRIPARTITE CLASSIFICATION: BUYERS
# ────────────────────────────────────────────────────────────
...
def classify_buyer(
    ...
    profile = BuyerProfile(
        ...
        total_projects_executed=buyer_data.get("total_projects_executed", 0),
        largest_project_value_egp=buyer_data.get("largest_project_value_egp", 0),
        total_acquisition_value_egp=buyer_data.get("total_acquisition_value_egp", 0),
    )
    profile.classify()
    return profile


def classify_all_buyers(
    user_accounts: List[Dict],
) -> List[Dict]:
    ...
    for account in user_accounts:
        ...
        results.append({
            ...
            "company_name": profile.company_name,
            "category": profile.buyer_category.value,
            "scale_label": profile.scale_label,
            "projects_executed": profile.total_projects_executed,
            "largest_project_egp": profile.largest_project_value_egp,
            "total_acquisition_egp": profile.total_acquisition_value_egp,
        })
...
# ────────────────────────────────────────────────────────────
# 6. FORMAT MATCHED CONTEXT FOR GLM ADVISORY REPORT
# ────────────────────────────────────────────────────────────

def format_match_results_for_llm(
    ...
    lines = [
        "MATCHED LAND RECORDS (Investment Rating & Matchmaking System)\n"
        + "=" * 70,
        "",
    ]
    ...
    for i, match in enumerate(matches, 1):
        ...
        score_breakdown = (
            f"  Score Breakdown:\n"
            f"    Usage Match      : {sd.get('usage', 0):.1f}/25\n"
            f"    Area Adequacy    : {sd.get('area', 0):.1f}/20\n"
            f"    Price Fit        : {sd.get('price', 0):.1f}/20\n"
            f"    Utility Coverage : {sd.get('utilities', 0):.1f}/15 ({match.utility_coverage_pct:.0f}%)\n"
            f"    Quality Rating   : {sd.get('quality', 0):.1f}/10 ({match.land_quality_rating})\n"
            f"    Auction/Location : {sd.get('auction', 0):.1f}/5 + {sd.get('governorate', 0):.1f}/5"
        )

        lines.append(
            f"--- Match #{i} | Compatibility: {match.compatibility_pct:.1f}% | "
            f"Quality: {match.land_quality_rating} ---\n"
            f"  Land ID        : {match.land_id}\n"
            f"  Location       : {land.get('Governorate', 'N/A')} - {land.get('Region_City', 'N/A')}\n"
            f"  Area           : {land.get('Total_Area_Sqm', 0):,} sqm\n"
            f"  Price/sqm      : {land.get('Price_Per_Sqm_EGP', 0):,} EGP\n"
            f"  Total Price    : {land.get('Total_Price_EGP', 0):,.0f} EGP\n"
            f"  Allowed Usage  : {land.get('Allowed_Usage', 'N/A')}\n"
            f"  Utilities      : {land.get('Utilities_Availability', 'N/A')}\n"
            f"  Highways       : {land.get('Nearest_Highways', 'N/A')}"
            f"{auction_info}\n"
            f"\n{score_breakdown}\n"
            f"\n  Strengths: {'; '.join(match.match_reasons)}\n"
            f"  Warnings : {'; '.join(match.gap_warnings) if match.gap_warnings else 'None'}\n"
            f"  Gov Notes: {land.get('Gov_Feasibility_Notes', 'N/A')}\n"
        )
