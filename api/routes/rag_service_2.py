...
class QueryIntent:
    ...
    def has_filters(self) -> bool:
        ...
        return bool(
            self.target_usage or self.target_gov or
            self.utility_keywords or self.transport_keywords or
            self.min_area or self.max_price
        )
...
USAGE_ALIASES: Dict[str, str] = {
    "industrial": "Industrial", "factory": "Industrial", "factories": "Industrial",
    "manufacturing": "Industrial", "plant": "Industrial",
    "logistics": "Logistics", "warehouse": "Logistics", "warehousing": "Logistics",
    "storage": "Logistics", "distribution": "Logistics", "freight": "Logistics",
    "agricultural": "Agricultural", "farming": "Agricultural", "agriculture": "Agricultural",
    "crops": "Agricultural", "irrigation": "Agricultural",
    "residential": "Residential", "housing": "Residential", "compound": "Residential",
    "apartments": "Residential", "living": "Residential",
}

GOVERNORATE_ALIASES: Dict[str, str] = {
    "cairo": "Cairo", "new cairo": "Cairo", "new administrative capital": "Cairo",
    "sharqia": "Sharqia", "10th of ramadan": "Sharqia", "tenth of ramadan": "Sharqia",
    "monufia": "Monufia", "sadat": "Monufia", "sadat city": "Monufia",
    "alexandria": "Alexandria", "borg el arab": "Alexandria",
    "suez": "Suez", "sokhna": "Suez",
    "damietta": "Damietta",
    "ismailia": "Ismailia",
    "aswan": "Aswan", "toshka": "Aswan",
    "beheira": "Beheira", "wadi el natrun": "Beheira",
}

UTILITY_KEYWORDS: List[str] = [
    "water", "electricity", "gas", "fiber", "fiber-optic",
    "optic", "solar", "grid", "pipeline", "sewage",
]

TRANSPORT_KEYWORDS: List[str] = [
    "highway", "road", "port", "airport", "canal", "tunnel",
    "desert road", "ring road", "corridor",
]
...
class RAGService:
    ...
    def search(
        ...
        logger.info(
            "RAG search: query='%s', intent=%s, results=%d",
            query[:60], {
                "usage": intent.target_usage,
                "gov": intent.target_gov,
    ...
    def extract_intent(self, query: str) -> QueryIntent:
        ...
        area_match = re.search(
            r"(?:at least|minimum|min|more than|over|above)\s*"
            r"([\d_]+)\s*(?:sqm|sq m|square meters?|m2)?",
            normed,
        )
        if area_match:
        ...
        price_match = re.search(
            r"(?:under|below|max|budget|cheaper than|up to)\s*"
            r"(?:egp\s*)?([\d_]+)\s*(?:egp|per\s*sqm)?",
            normed,
        )
        if price_match:
    ...
    def rerank(
        ...
        for land_dict, base_score in results:
            ...
            # Boost for partial region name match
            ...
            # Boost for soil/mineral keyword match
            ...
            # Boost for highway proximity mentions in feasibility notes
    ...
    def build_context(self, results: List[Tuple[Dict[str, Any], int]]) -> str:
        ...
        lines = [
            "RETRIEVED LAND RECORDS (Egypt Land Database)",
            "=" * 60,
        ]
        ...
        for item in results:
            ...
            lines.append(
                ...
                f"  Location       : {land['Governorate']} - {land['Region_City']}\n"
                f"  Coordinates    : {land['Latitude']}, {land['Longitude']}\n"
                f"  Area           : {land['Total_Area_Sqm']:,} sqm\n"
                f"  Price/sqm      : {land['Price_Per_Sqm_EGP']:,} EGP\n"
                f"  Soil/Mineral   : {land['Soil_Mineral_Type']}\n"
                f"  Allowed Usage  : {land['Allowed_Usage']}\n"
                f"  Highways       : {land['Nearest_Highways']}\n"
                f"  Utilities      : {land['Utilities_Availability']}"
                f"{auction_info}\n"
                f"  Gov Notes      : {land['Gov_Feasibility_Notes']}\n"
            )
