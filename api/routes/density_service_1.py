...
# ────────────────────────────────────────────────────────────
# Saturation Thresholds (based on 5km primary analysis)
# ────────────────────────────────────────────────────────────

_SATURATION_THRESHOLDS: Dict[str, Tuple[int, int, int]] = {
    "retail": (3, 7, 12),
    "civic": (2, 5, 8),
    "industrial": (3, 8, 15),
}

# Thresholds: (LOW_MAX, MODERATE_MAX, HIGH_MAX)
# >= HIGH_MAX → CRITICAL
...
_GAP_RECOMMENDATIONS: Dict[str, List[Dict]] = {
    "Retail & Entertainment": [
        {"usage": "Hospital / Medical Center", "rationale": "Low civic density creates healthcare demand"},
        {"usage": "School / University Campus", "rationale": "Residential population needs education facilities"},
        {"usage": "Logistics Hub", "rationale": "Industrial gap supports supply-chain demand"},
        {"usage": "Residential Compound", "rationale": "Population density justifies housing projects"},
    ],
    "Civic Infrastructure": [
        {"usage": "Shopping Mall / Retail Complex", "rationale": "Underserved retail market for existing population"},
        {"usage": "Entertainment / Amusement Park", "rationale": "No entertainment venues in proximity"},
        {"usage": "Medical Facility", "rationale": "Healthcare gap in the area"},
        {"usage": "Industrial Zone", "rationale": "No industrial competition, first-mover advantage"},
    ],
    "Industrial Infrastructure": [
        {"usage": "Residential Compound", "rationale": "No industrial proximity — clean environment for housing"},
        {"usage": "School / University", "rationale": "Quiet zone suitable for educational campus"},
        {"usage": "Hospital / Medical Center", "rationale": "Clean air and low pollution support healthcare facilities"},
        {"usage": "Retail & Entertainment", "rationale": "Unsaturated retail market for worker/resident population"},
    ],
}
...
class DensityService:
    ...
    def analyze(self, land: Dict) -> ServiceDensityData:
        ...
        return ServiceDensityData(
            profiles=profiles,
            retail_entertainment_saturation=retail_sat,
            civic_infrastructure_saturation=civic_sat,
            industrial_infrastructure_saturation=industrial_sat,
            overall_density_score=overall_score,
    ...
    def generate_saturation_report(self, land: Dict) -> str:
        ...
        lines = [
            f"MARKET SATURATION & GAP REPORT — {land_id} ({region})",
            "=" * 55,
            f"Land Usage Type: {usage}",
            "",
        ...
        for p in analysis.profiles:
            lines.append(
                f"  Within {p.radius_km:.0f}km: "
                f"Retail/Entertainment: {p.retail_entertainment} | "
                f"Civic Infrastructure: {p.civic_infrastructure} | "
                f"Industrial: {p.industrial_infrastructure} | "
                f"Total: {p.total}"
            )

        lines.extend([
            "",
            "SATURATION ASSESSMENT (based on {0:.0f}km radius):".format(self.primary_radius_km),
            f"  Retail & Entertainment: {analysis.retail_entertainment_saturation.value}",
            f"  Civic Infrastructure:    {analysis.civic_infrastructure_saturation.value}",
            f"  Industrial:              {analysis.industrial_infrastructure_saturation.value}",
            f"  Overall Density Score:   {analysis.overall_density_score:.1f}/100",
            "",
            "CLUSTERING VERDICT:",
        ...
        if analysis.retail_entertainment_saturation in (SaturationLevel.HIGH, SaturationLevel.CRITICAL):
            lines.extend([
                "",
                "SATURATION WARNING:",
                f"  This area shows {analysis.retail_entertainment_saturation.value} retail/entertainment saturation. "
                "Investing in another mall, plaza, or similar retail venue carries elevated competition risk. "
                "Strongly consider the alternative usages identified above, or target a niche retail segment "
                "(e.g., medical mall, edu-tainment center) to differentiate from existing supply.",
            ])

        if analysis.civic_infrastructure_saturation in (SaturationLevel.HIGH, SaturationLevel.CRITICAL):
            lines.extend([
                "",
                "CIVIC SATURATION NOTICE:",
                f"  Civic infrastructure is {analysis.civic_infrastructure_saturation.value} in this area. "
                "Additional hospitals, schools, or universities may face enrollment/admission challenges. "
                "Evaluate demographic growth projections before committing to civic-type development.",
            ])

        if analysis.industrial_infrastructure_saturation in (SaturationLevel.HIGH, SaturationLevel.CRITICAL):
            lines.extend([
                "",
                "INDUSTRIAL SATURATION NOTICE:",
                f"  Industrial density is {analysis.industrial_infrastructure_saturation.value}. "
                "New factories or warehouses may compete for labor and logistics capacity. "
                "Consider specialized industrial segments or alternative sectors listed above.",
            ])

        return "\n".join(lines)

    # ────────────────────────────────────────────────────────
    # Density-to-Context Formatter (for RAG/LLM pipeline)
    # ────────────────────────────────────────────────────────
    ...
    def format_density_for_llm(self, land: Dict) -> str:
        ...
        parts = [
            f"  Service Density ({self.primary_radius_km:.0f}km): "
            f"Retail={primary.retail_entertainment} | "
            f"Civic={primary.civic_infrastructure} | "
            f"Industrial={primary.industrial_infrastructure} | "
            f"Score={analysis.overall_density_score:.0f}/100",
            f"  Saturation: Retail={analysis.retail_entertainment_saturation.value}, "
            f"Civic={analysis.civic_infrastructure_saturation.value}, "
            f"Industrial={analysis.industrial_infrastructure_saturation.value}",
        ]
        ...
        if analysis.market_gap_analysis:
            ...
            parts.append(
                f"  Top Gap Opportunity: {top_gap['recommended_usage']} "
                f"({top_gap['rationale']})"
            )
    ...
    def _build_profiles(raw_profiles: List[Dict]) -> List[RadiusProfile]:
        ...
        for rp in raw_profiles:
            ...
            profiles.append(RadiusProfile(
                radius_km=rp["radius_km"],
                retail_entertainment=retail,
    ...
    def _classify_saturation(
        self, profiles: List[RadiusProfile],
    ) -> Tuple[SaturationLevel, SaturationLevel, SaturationLevel]:
        """
        ...
        retail_sat = self._classify_single(
            primary.retail_entertainment, _SATURATION_THRESHOLDS["retail"]
        )
        civic_sat = self._classify_single(
            primary.civic_infrastructure, _SATURATION_THRESHOLDS["civic"]
        )
        industrial_sat = self._classify_single(
            primary.industrial_infrastructure, _SATURATION_THRESHOLDS["industrial"]
        )
    ...
    def _compute_overall_score(self, profiles: List[RadiusProfile]) -> float:
        ...
        # Max expected density at 5km for scoring normalization
        max_density = {
            "retail": 15.0,
            "civic": 10.0,
            "industrial": 20.0,
        ...
        raw_score = (
            retail_norm * 35.0
            + civic_norm * 30.0
            + industrial_norm * 35.0
        )
        return round(raw_score, 1)

    def _identify_gaps(
        self,
        retail_sat: SaturationLevel,
        civic_sat: SaturationLevel,
        industrial_sat: SaturationLevel,
        land: Dict,
    ) -> List[Dict]:
        """
        ...
        saturation_map = {
            "Retail & Entertainment": retail_sat,
            "Civic Infrastructure": civic_sat,
            "Industrial Infrastructure": industrial_sat,
        }
        ...
        for category, sat_level in saturation_map.items():
            if sat_level in (SaturationLevel.HIGH, SaturationLevel.CRITICAL):
                ...
                for rec in recommendations:
                    viable_note = None
                    # Check if the land's own usage aligns with the gap
                    if self._usage_matches_recommendation(usage, rec["usage"]):
                        viable_note = (
                            f"Your planned {usage} usage aligns with this gap — "
                            "competitive positioning is favorable."
                        )
                    elif self._usage_matches_saturated_category(usage, category):
                        viable_note = (
                            f"WARNING: Your planned {usage} usage falls in the "
                            f"'{category}' category which is at {sat_level.value} "
                            "saturation. Strongly consider pivoting to one of the "
                            "recommended alternatives."
                        )

                    gaps.append({
                        "saturated_category": category,
                        "saturation_level": sat_level.value,
                        "recommended_usage": rec["usage"],
                        "rationale": rec["rationale"],
                        "current_usage_viable": viable_note,
                    })

        # Sort: warnings about current usage first, then opportunities
    ...
    def _usage_matches_recommendation(usage: str, recommendation: str) -> bool:
        ...
        keyword_map = {
            ...
            "residential": ["residential", "housing", "compound", "apartment"],
            "logistics": ["logistics", "warehouse", "distribution", "hub"],
            "agricultural": ["agricultural", "farming", "agriculture", "crop"],
        }
    ...
    def _usage_matches_saturated_category(usage: str, category: str) -> bool:
        ...
        if "retail" in category.lower() or "entertainment" in category.lower():
            return False  # Retail/entertainment is not a land usage type in the system
        if "civic" in category.lower():
            civic_kw = ["residential"]  # Residential benefits from civic, but isn't civic itself
            return False
    ...
    def _generate_verdict(
        ...
        if overall_score >= 75:
            verdict = (
                "Highly clustered zone. Established commercial ecosystem with "
                "intense competition. Due diligence on market positioning required."
            )
        elif overall_score >= 50:
            verdict = (
                "Moderately developed cluster. Existing service base present with "
                "selective opportunities for differentiated development."
            )
        elif overall_score >= 25:
            verdict = (
                "Lightly clustered area. Emerging service landscape with "
                "first-mover advantages in several categories."
            )
        else:
            verdict = (
                "Undeveloped zone with minimal existing services. Greenfield opportunity "
                "but requires significant infrastructure investment to attract demand."
            )

        # Add usage-specific warning if applicable
