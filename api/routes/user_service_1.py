...
from models.user import (
    UserAccount, UserRole, BrokerVerificationStatus,
    DocumentType, BrokerDocument,
)
...
class UserService:
    ...
    REQUIRED_BROKER_DOCUMENT_TYPES = [
        DocumentType.BROKERAGE_LICENSE,
        DocumentType.FINANCIAL_GUARANTEE,
    ]
    ...
    def submit_broker_document(
        self,
        broker_id: str,
        document_type: DocumentType,
        document_name: str = "",
    ) -> Optional[BrokerDocument]:
        """
    ...
    def _check_broker_verification_readiness(self, broker_id: str) -> None:
        ...
        verified_types = {
            doc.document_type for doc in user.broker_documents if doc.verified
        }
        ...
        if required.issubset(submitted_types) and required.issubset(verified_types):
            ...
            has_rejection = any(
                doc.rejection_reason for doc in user.broker_documents
                if not doc.verified and doc.document_type in required
            )
            if has_rejection:
    ...
    def get_broker_verification_status(self, broker_id: str) -> Optional[Dict]:
        ...
        return {
            ...
            "status": user.broker_verification_status.value,
            "required_documents": [t.value for t in required],
            "submitted": [t.value for t in submitted_types],
            "verified": [t.value for t in verified_types],
            "missing": [t.value for t in required - submitted_types],
            "pending_verification": [t.value for t in submitted_types - verified_types],
            "can_access_dashboard": user.can_access_dashboard,
            "can_manage_listings": user.can_manage_listings,
            "documents": [
                {
                    "document_id": d.document_id,
                    "type": d.document_type.value,
                    "name": d.document_name,
                    "uploaded": d.upload_date[:10] if d.upload_date else "",
                    "verified": d.verified,
                    "verified_date": (d.verification_date[:10] if d.verification_date else ""),
    ...
    def record_broker_deal(
        ...
        if user.assigned_land_ids:
            ...
            user.performance_score = round(
                min(100.0, (user.total_deals_closed / max(active_count, 1)) * 50
                + (user.total_commission_earned_egp / 1_000_000) * 10),
                2,
            )
        return user
...
def _seed_sample_users(svc: UserService) -> None:
    ...
    buyer = svc.create_account(
        full_name="Karim El-Masry",
        role=UserRole.BUYER_INVESTOR,
        email="karim@nilecap.eg",
        phone="+201012345678",
        company_name="Nile Capital Partners",
        investment_budget_max_egp=500_000_000,
        preferred_usages=["Industrial", "Logistics"],
        preferred_governorates=["Cairo", "Suez", "Ismailia"],
    )
    svc.add_to_watchlist(buyer.user_id, "EG-CAI-01")
    svc.add_to_watchlist(buyer.user_id, "EG-SUZ-01")

    seller = svc.create_account(
        full_name="Nadia Fawzy",
        role=UserRole.SELLER_OWNER,
        email="nadia@gulfreal.eg",
        phone="+201098765432",
        company_name="Gulf Real Estate Holdings",
    )
    svc.register_owned_land(seller.user_id, "EG-CAI-01")

    broker = svc.create_account(
        full_name="Omar Abdel-Rahim",
        role=UserRole.CERTIFIED_BROKER,
        email="omar@certifiedbroker.eg",
        phone="+201155544433",
        company_name="Abdel-Rahim Real Estate",
        broker_license_number="BROK-EG-2024-0447",
        broker_specializations=["Industrial", "Logistics", "Commercial"],
    )
    ...
    svc.create_account(
        ...
        role=UserRole.CERTIFIED_BROKER,
        email="hassan@realestate.eg",
        phone="+201234567890",
        company_name="Ibrahim Properties",
        broker_license_number="BROK-EG-2024-0891",
        broker_specializations=["Residential", "Commercial"],
    )