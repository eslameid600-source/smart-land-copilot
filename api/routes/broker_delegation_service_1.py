...
class BrokerDelegationService:
    ...
    def allocate_broker(
        ...
        broker_id: str,
        broker_name: str = "",
    ) -> Tuple[Optional[BrokerAllocation], str]:
        """
        ...
        if len(current) >= self.MAX_BROKERS_PER_LAND:
            ...
            return None, (
                f"Maximum {self.MAX_BROKERS_PER_LAND} brokers already allocated "
                f"to {land_id}: {', '.join(existing_names)}"
            )
        ...
        logger.info(
            f"Allocated broker {broker_name} ({broker_id}) to land {land_id} "
            f"({len(current)}/{self.MAX_BROKERS_PER_LAND})"
        )
        return allocation, ""
    ...
    def close_deal(
        self,
        land_id: str,
        winning_broker_id: str,
        buyer_id: str,
        transaction_value_egp: float,
        broker_commission_pct: float = 1.5,
    ) -> Optional[BrokerCommissionRecord]:
        """
        ...
        if winning_broker:
            ...
            broker_list.append({
                "broker_id": winning_broker.broker_id,
                "broker_name": winning_broker.broker_name,
                "is_winning": True,
                "commission_egp": winning_commission,
                "leads_generated": winning_broker.leads_generated,
                "deals_closed": winning_broker.deals_closed,
            })
        if secondary_broker:
            broker_list.append({
                "broker_id": secondary_broker.broker_id,
                "broker_name": secondary_broker.broker_name,
                "is_winning": False,
                "commission_egp": 0.0,
                "leads_generated": secondary_broker.leads_generated,
                "deals_closed": secondary_broker.deals_closed,
            })

        record = BrokerCommissionRecord(
            land_id=land_id,
            transaction_value_egp=transaction_value_egp,
            broker_commission_pct=broker_commission_pct,
            allocated_brokers=broker_list,
            winning_broker_id=winning_broker_id,
            winning_broker_commission_egp=winning_commission,
            secondary_broker_id=secondary_broker.broker_id if secondary_broker else None,
            secondary_broker_commission_egp=0.0,
            deal_closed=True,
        ...
        logger.info(
            f"Deal closed for {land_id}: Winner={winning_broker_id}, "
            f"Commission={winning_commission:,.0f} EGP, Secondary=0 EGP"
        )
        return record
    ...
    def get_broker_performance_summary(self, broker_id: str) -> Dict:
        ...
        return {
            "broker_id": broker_id,
            "lands_assigned": lands_assigned,
            "total_leads_generated": total_leads,
            "total_deals_closed": total_deals,
            "lands_won": lands_won,
            "total_commission_earned_egp": total_commission,
            "win_rate_pct": round((lands_won / max(lands_assigned, 1)) * 100, 1),
        }
...
def _seed_sample_delegations(svc: BrokerDelegationService) -> None:
    ...
    if len(brokers) >= 2:
        ...
        if buyer:
            svc.close_deal(
                land_id="EG-SUZ-01",
                winning_broker_id=brokers[0].user_id,
                buyer_id=buyer.user_id,
                transaction_value_egp=462_000_000,
