...
from models.land import USAGE_COLORS, ALL_UTILITIES
...
def render_metric_card(label: str, value: str, color: str = "#ecf0f1") -> None:
    ...
    st.markdown(
        ...
        f'<div style="font-size:16px;font-weight:bold;color:{color};">{value}</div>'
        f'</div>',
...
def render_compatibility_bar(percent: float, height: int = 22) -> None:
    ...
    st.markdown(
        f"""<div style="background:#1e1e1e;border-radius:8px;height:{height}px;overflow:hidden;">
...
def render_auction_card(land: Dict[str, Any]) -> None:
    ...
    st.markdown(
        f"""<div style="background:linear-gradient(135deg,#3d1f00,#5c2e00);
    ...
    if breakdown:
        ...
        for sb in breakdown:
            ...
            rows += (
                ...
                f'<span style="font-size:10px;color:#888;width:90px;">{sb.get("category", "")}</span>'
                f'<div style="flex:1;background:#1e1e1e;border-radius:4px;height:8px;overflow:hidden;">'
                f'<div style="width:{min(pct, 100)}%;height:100%;background:{c};border-radius:4px;"></div>'
                f'</div>'
                f'<span style="font-size:10px;color:#aaa;width:40px;text-align:right;">{pct:.0f}%</span>'
                f'</div>'
            )
