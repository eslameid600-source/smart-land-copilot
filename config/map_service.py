...
from models.models.models.land import USAGE_COLORS, LandRecord
...
class MapService:
    ...
    def build_map(self, usage_filter: Optional[str]=None, highlight_land_id: Optional[str]=None) -> folium.Map:
        """
        ...
        mc = self._mc
        egypt_map = folium.Map(location=[mc.default_center_lat, mc.default_center_lon], zoom_start=mc.default_zoom, tiles=None, control_scale=True)
        folium.raster_layers.TileLayer(tiles=mc.tile_url, attr=mc.tile_attribution, name='Dark Mode', max_zoom=19, overlay=False, control=True).add_to(egypt_map)
        ...
        if mc.enable_clustering and len(lands) > 3:
            cluster = MarkerCluster(name='Land Parcels', options={'maxClusterRadius': mc.cluster_radius}).add_to(egypt_map)
        for land in lands:
            ...
            if land.is_auction:
                folium.CircleMarker(location=[land.latitude, land.longitude], radius=land.radius_meters / 1000 + 0.15, color='#f39c12', fill=False, weight=2, dash_array='8, 6', tooltip=f'AUCTION: {land.auction_date}').add_to(egypt_map)
            if highlight_land_id and land.land_id == highlight_land_id:
                folium.CircleMarker(location=[land.latitude, land.longitude], radius=land.radius_meters / 1000 + 0.3, color='#00ff88', fill=False, weight=3, tooltip=f'Selected: {land.land_id}').add_to(egypt_map)
            folium.Marker(location=[land.latitude + 0.15, land.longitude + 0.15], icon=folium.DivIcon(html=f'<div style="font-size:9px;color:{color};white-space:nowrap;text-shadow:1px 1px 3px #000;font-weight:bold;">{land.land_id}</div>', icon_size=(80, 20), icon_anchor=(0, 10))).add_to(egypt_map)
    ...
    def find_nearest_land(self, lat: float, lon: float, max_distance_deg: float=1.0) -> Optional[Dict[str, Any]]:
