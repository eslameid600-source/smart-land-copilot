...
# ── Reference Data: Egyptian Parks & Green Spaces ──
# Approximate distances used for scoring when real geospatial
# data is not available. In production, this would use a GIS API.

EGYPTIAN_GREEN_SPACES = {
    "Cairo": {
        "parks": [
            {"name": "Al-Azhar Park", "lat": 30.0457, "lon": 31.2620, "area_ha": 30},
            {"name": "Family Park New Cairo", "lat": 30.0160, "lon": 31.4400, "area_ha": 20},
            {"name": "International Garden", "lat": 30.0580, "lon": 31.3310, "area_ha": 35},
            {"name": "Orman Botanical Garden", "lat": 30.0195, "lon": 31.2092, "area_ha": 8},
            {"name": "Gabalaya Park", "lat": 30.0530, "lon": 31.2180, "area_ha": 5},
        ],
    },
    "Giza": {
        "parks": [
            {"name": "Giza Zoo & Gardens", "lat": 30.0284, "lon": 31.2158, "area_ha": 34},
            {"name": "Fustat Gardens", "lat": 30.0050, "lon": 31.2360, "area_ha": 12},
            {"name": "Oasis Park 6th October", "lat": 29.9590, "lon": 30.9010, "area_ha": 15},
        ],
    },
    "Alexandria": {
        "parks": [
            {"name": "Montaza Palace Gardens", "lat": 31.2985, "lon": 30.0193, "area_ha": 60},
            {"name": "Antoniadis Garden", "lat": 31.2030, "lon": 29.9080, "area_ha": 40},
            {"name": "Alexandria Zoo", "lat": 31.2089, "lon": 29.9103, "area_ha": 10},
        ],
    },
    "Suez": {
        "parks": [
            {"name": "Suez Public Park", "lat": 29.9668, "lon": 32.5498, "area_ha": 8},
        ],
    },
    "Ismailia": {
        "parks": [
            {"name": "Ismailia Green Island", "lat": 30.5920, "lon": 32.2710, "area_ha": 12},
            {"name": "Ismailia Public Garden", "lat": 30.5950, "lon": 32.2650, "area_ha": 6},
        ],
    },
}
...
class EnvironmentalService:
    ...
    def compute_greenery_index(self, land_dict: Dict) -> GreeneryDensityData:
        ...
        if not parks:
            return GreeneryDensityData(
                greenery_density_index=0.0,
                greenery_verdict=f"No green space data available for {governorate}",
            )
        ...
        # Proximity factor: inverse distance, max 40 pts
        ...
        # Park count factor: max 30 pts
        count_score = min(parks_5km * 8, 30.0)

        # Green area factor: max 30 pts (logarithmic scale)
        if total_green_ha > 0:
        ...
        return GreeneryDensityData(
            nearest_park_name=nearest["name"],
            nearest_park_distance_km=round(nearest["distance_km"], 2),
            parks_within_2km=parks_2km,
            parks_within_5km=parks_5km,
            total_green_area_hectares=round(total_green_ha, 1),
            greenery_density_index=index,
            greenery_verdict=verdict,
        )

    def compute_creator_studio_score(
        self,
        land_dict: Dict,
        greenery: GreeneryDensityData,
        ...
        # ── Factor 1: Greenery / Low-Noise Environment (max 50 pts) ──
        greenery_score = greenery.greenery_density_index

        # Infer noise from land usage and location
        ...
        # ── Factor 2: Fiber Optic Infrastructure (max 50 pts) ──
        ...
        return CreatorStudioSuitability(
            suitability_score=total_score,
            noise_level_rating=noise_rating,
            greenery_factor=greenery_factor,
            fiber_optic_factor=fiber_factor,
            fiber_optic_available=has_fiber,
            internet_speed_mbps=internet_speed,
            power_stability_rating=power_stability,
